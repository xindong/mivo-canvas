...
```json
{
  "lastGithash": "<上次非空补扫时的 origin/main HEAD；空跑不前移，由下次非空补扫追平，见第 7 步>",
...
- `features`/`fixes` 的每条是 `{ text, by }`：`by` = 该 PR 的**作者**（PR opener，`gh pr view --json author` 的 login；语义=「这个功能谁做的」，最贴合 changelog）。`gh` 不可用时降级链 = PR opener → `^2` 分支作者（merge 形态）→ `%an`（squash 形态）。前端把 `by` 渲染成条目尾部的灰字小标签（空串则不显示）。旧版纯 string 条目前端可向后兼容读取，但**新写入一律用对象形态**
...
- 每条填 `by` = 该 PR 的**作者**（PR opener，`gh pr view --json author` 的 login；`gh` 不可用时降级到 `^2` 分支作者 / `%an`）；一个 PR 拆出的多条同 `by`，多个 PR 合成的一条取主 PR 的作者
...
- `updatedAt` **仅在实际有新条目写入时刷新**——无新 PR 的空跑不改 `updatedAt`（避免无意义红点）
- **空跑不前移 `lastGithash`**（脚本化后既定决策,2026-07-08）：空跑时 changelog.json 不写回、anchor 不动,由**下次非空补扫**（手动跑本 skill 或 `auto-changelog.mjs publish`）把 `lastGithash` 跳到当时 `origin/main` HEAD 自然追平。理由:GitHub Actions 是唯一 meta-PR 创建者,空跑日意味着无新真 PR 落地（否则就非空了）→ 无 commit 落地,anchor 滞后恒 ≤1 个 meta-PR（被 subject 跳过）且不累积,不值得为 bump 一个字段每天开 PR 烧 6 项 CI。详见 `scripts/changelog/RUNBOOK.md`。
...
- 本 skill 不做运行时 git——生成完全发生在开发态；自动模式由 GitHub Actions 在 runner 上跑 `scripts/changelog/auto-changelog.mjs`

### 自动模式（每日 8:00 GitHub Actions 无人值守调用）

> **自动闭环已由脚本承载（2026-07-09）**：本模式的 scan / publish 机械步骤（fetch / git log 差集 / 归天 / 去重 / 建临时 worktree / 合并写回 / 开 PR / 轮询 CI / squash merge / 清理）是确定性脚本；rewrite 由同一脚本直调公司 LLM 网关（OpenAI-compatible `/v1/chat/completions`），不再拉起 maker 调度器 agent。GitHub Actions 每日 8:00 跑三步：
>
> 1. `node scripts/changelog/auto-changelog.mjs scan` — 扫出新 PR，产物落 `/tmp/mivo-changelog-scan.json`
> 2. `node scripts/changelog/auto-changelog.mjs rewrite` — 脚本先确定性生成 date/by/kind 骨架（`feat`→`features`,`fix`→`fixes`,其他前缀低调归入 `fixes`），LLM 只写 PR→文案映射，产物落 `/tmp/mivo-changelog-rewrite.json`
> 3. `node scripts/changelog/auto-changelog.mjs publish --rewrite /tmp/mivo-changelog-rewrite.json` — 脚本回填机器字段、校验 + 开 PR + 轮询 CI + squash merge
>
> 三步编排详见 `scripts/changelog/RUNBOOK.md`。脚本行为与下方原 SOP 语义一致；下面保留原 SOP 作为**语义规格说明**，供审查脚本实现是否忠实于本 skill 时对照。

由 `.github/workflows/daily-changelog.yml` 每日 8:00 触发（恰在结算日翻页后），无人值守跑完整补扫闭环。完整 SOP：
...
2. **无新 PR → 成功退出**：补扫差集为空（changelog.json 无实质变化，`updatedAt` 不刷新，`lastGithash` 也不前移——见第 7 步既定决策，下次非空 publish 自然追平）时，**不开 PR、不留分支**，Actions 绿灯退出
3. **有新条目 → 开 PR**：
   ```bash
...
6. **安全铁律（一条都不能破）**：
   ...
   - CI 未全绿 / 出现冲突 / 等待超过合理时长（建议 30 分钟）→ **不 merge**，workflow 红灯，保留 PR 待人工处理后退出
   - 任何步骤失败（fetch 失败、push 被拒、gh 报错）同样 workflow 红灯，不做破坏性重试
