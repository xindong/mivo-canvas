...
- **空跑不前移 `lastGithash`**（脚本化后既定决策,2026-07-08）：空跑时 changelog.json 不写回、anchor 不动,由**下次非空补扫**（手动跑本 skill 或 `auto-changelog.mjs publish`）把 `lastGithash` 跳到当时 `origin/main` HEAD 自然追平。理由:本调度器是唯一 meta-PR 创建者,空跑日意味着无新真 PR 落地（否则就非空了）→ 无 commit 落地,anchor 滞后恒 ≤1 个 meta-PR（被 subject 跳过）且不累积,不值得为 bump 一个字段每天开 PR 烧 6 项 CI。详见 `scripts/changelog/RUNBOOK.md`。
...
2. **无新 PR → 静默退出**：补扫差集为空（changelog.json 无实质变化，`updatedAt` 不刷新，`lastGithash` 也不前移——见第 7 步既定决策，下次非空 publish 自然追平）时，**不开 PR、不留分支**，调用 `schedule_silence_current_run`（lizi_scheduler MCP）静默本次运行，然后清理临时 worktree 退出
3. **有新条目 → 开 PR**：
   ```bash
