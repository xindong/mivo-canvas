...
| `MIVO_DEBUG_RETENTION_DAYS` | `7` | `server/lib/debug-records.ts:193` | debug-logs 过期 .jsonl 保留天数;append 前惰性 sweep(日期早于 today-N);**`0`=禁用 sweep(无限保留)**,与 `MIVO_DEBUG_DISK_QUOTA_MB` 的 `0=禁用` 同义(避免误设 0 清空全部日志) |
