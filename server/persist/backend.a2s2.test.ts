...
// 范围:直接调 InMemoryPersistBackend(PG 实装后双跑,gated MIVO_PG_TEST);不走 HTTP route(route 层在 Block 5)。
//
// 冻结矩阵(§14.1):
...
import { InMemoryPersistBackend } from './backend'
...
const seedCanvas = async (b: InMemoryPersistBackend, ownerId = 'owner', canvasId = 'c1'): Promise<void> => {
...
describe('A2-S2 §14.1 冻结矩阵 — InMemoryPersistBackend', () => {
  beforeAll(() => setBaseCursorSecrets(['test-secret-a2s2']))
