...
describe('G2.1 R2-2 — ssoStrictProofGate 前置 proof(body 解析/DB lookup 前,存在性 oracle 消除)', () => {
  ...
  beforeEach(() => {
    ;({ app, backend, permissions } = buildPersistApp())
  })
  afterEach(() => vi.unstubAllEnvs())
  ...
  // R3-F2:token presence 不再豁免(存在≠proof)。garbage/revoked/expired token → 不豁免 → 401(无 proof);
  // 仅 valid active token 豁免 → route authz 验 token↔project → 公开分享访问(200)。
  it('strict + 无 proof + garbage share token → 401(R3-F2:存在≠proof,不再豁免;返修前 fake-token → 404 绕过)', async () => {
    vi.stubEnv('MIVO_SSO_STRICT', '1')
    vi.stubEnv('MIVO_GATEWAY_SECRET', GW)
    const res = await req(app, '/api/projects/some-id', {
      headers: { 'x-mivo-share-token': 'fake-token' },
    })
    expect(res.status).toBe(401) // R3-F2:garbage token 不豁免 → 落 proof → 401(返修前 404 绕过)
    expect((res.body as { error: string }).error).toBe('unauthorized')
  })

  it('strict + 无 proof + valid active share token → 豁免(public path;route authz 验 token↔project → 200)', async () => {
    // seed:owner(alice)建 project p1 + view share link(用本块共享 permissions,gate 同一 backend)
    await backend.ensureCreate('alice@xd.com', 'project', 'p1', { name: 'P1' }, { method: 'POST', resourceKind: 'project' })
    const link = await permissions.createShareLink('p1', 'view', 'alice@xd.com')
    vi.stubEnv('MIVO_SSO_STRICT', '1')
    vi.stubEnv('MIVO_GATEWAY_SECRET', GW)
    const res = await req(app, '/api/projects/p1', { headers: { 'x-mivo-share-token': link.token } })
    expect(res.status).toBe(200) // valid active token → 豁免 → route authz 验 token↔p1 → read 允许 → 200
  })

  it('strict + 无 proof + revoked share token → 401(revoked 不豁免,落 proof)', async () => {
    await backend.ensureCreate('alice@xd.com', 'project', 'p1', { name: 'P1' }, { method: 'POST', resourceKind: 'project' })
    const link = await permissions.createShareLink('p1', 'view', 'alice@xd.com')
    await permissions.revokeShareLink(link.id, 'p1')
    vi.stubEnv('MIVO_SSO_STRICT', '1')
    vi.stubEnv('MIVO_GATEWAY_SECRET', GW)
    const res = await req(app, '/api/projects/p1', { headers: { 'x-mivo-share-token': link.token } })
    expect(res.status).toBe(401) // revoked token → resolveShareLinkByToken kind='revoked' → 不豁免 → 401
  })
...
// ── G2.1 R3-F2:garbage share token 不再绕过 proof gate(各 owner route 收口)──────────────────────
// R3-F2[P1]复现:ssoStrictProofGate 只判 share header/query 非空就 next(),tasks/user-state/projects root
// 等不支持 share 的 owner route 可用垃圾 token 绕过前置 401,恢复未鉴权 body/multipart 解析(DoS)。
// 验收:projects root、user-state、tasks 各 POST 加 garbage share header + invalid/oversized body → 401
// 且 parser/backend 未调用(spy);assets invalid/missing/known 无 proof 均返回 401;valid token 仍走公开路径。
describe('G2.1 R3-F2 — garbage share token 不绕过 proof gate(按 route 能力收窄 + token 有效性校验)', () => {
  let app: ReturnType<typeof buildPersistApp>['app']
  let backend: ReturnType<typeof buildPersistApp>['backend']
  beforeEach(() => {
  ...
  it('strict + 无 proof + garbage token + POST /api/projects(根)非法 JSON → 401(非 400;ensureCreate 未调用)', async () => {
    vi.stubEnv('MIVO_SSO_STRICT', '1')
    vi.stubEnv('MIVO_GATEWAY_SECRET', GW)
    const spy = vi.spyOn(backend, 'ensureCreate')
    const res = await req(app, '/api/projects', {
      method: 'POST',
      headers: { 'content-type': 'application/json', 'x-mivo-share-token': 'garbage' },
      body: 'this-is-not-json',
    })
    expect(res.status).toBe(401) // R3-F2:garbage token 不豁免(projects root shareCapable=false)→ 落 proof → 401
    expect((res.body as { error: string }).error).toBe('unauthorized')
    expect(spy).not.toHaveBeenCalled() // body 未解析 → ensureCreate 未触达
  })

  it('strict + 无 proof + garbage token + POST /api/projects(根)超 1MB body → 401(非 413;body cap 未触达)', async () => {
    vi.stubEnv('MIVO_SSO_STRICT', '1')
    vi.stubEnv('MIVO_GATEWAY_SECRET', GW)
    const huge = 'x'.repeat(2 * 1024 * 1024)
    const res = await req(app, '/api/projects', {
    ...
    expect(res.status).toBe(401) // body cap 不触达;garbage token → 401 在 body 读前
  })

  it('strict + 无 proof + garbage token + PUT /api/user-state/:key 非法 JSON → 401(非 400;parser 未调用)', async () => {
    vi.stubEnv('MIVO_SSO_STRICT', '1')
    vi.stubEnv('MIVO_GATEWAY_SECRET', GW)
    const res = await req(app, '/api/user-state/camera:pos', {
      method: 'PUT',
      headers: { 'content-type': 'application/json', 'x-mivo-share-token': 'garbage' },
      body: 'not-json',
    })
    expect(res.status).toBe(401) // user-state shareCapable=false → garbage token 不豁免 → 401(namespace/body 未触达)
    expect((res.body as { error: string }).error).toBe('unauthorized')
  })

  it('strict + 无 proof + garbage token + POST /api/mivo/tasks/generate 非法 JSON → 401(非 400;parser 未调用)', async () => {
    __resetTaskRegistry()
    vi.stubEnv('MIVO_SSO_STRICT', '1')
    vi.stubEnv('MIVO_GATEWAY_SECRET', GW)
    const res = await realApp.request('/api/mivo/tasks/generate', {
    ...
    expect(res.status).toBe(401) // tasks shareCapable=false → garbage token 不豁免 → 401(readJsonBody 未调用)
  })

  it('strict + 无 proof + garbage token + GET /api/projects/:id(根无 :id → shareCapable=false)→ 401', async () => {
    // projects 根(无 :id)hasSubresourceId=false → garbage token 不豁免(返修前 :id 子资源 garbage 也绕过,根本不支持 share)
    vi.stubEnv('MIVO_SSO_STRICT', '1')
    vi.stubEnv('MIVO_GATEWAY_SECRET', GW)
    const listRes = await req(app, '/api/projects', { headers: { 'x-mivo-share-token': 'garbage' } })
    expect(listRes.status).toBe(401) // GET /api/projects(根 list)不支持 share → garbage token → 401
  })
})

// ── G2.1 R3-F2:assets 路由 resolveAssetOwner 移到所有 validation 前 ──────────────────────────────
// R3-F2 assets:返修前 GET /api/assets/not-a-hash strict+无 proof → 404(ASSET_ID_RE 在 resolveAssetOwner 前);
// GET /api/assets/<64hex missing> → 401(resolveAssetOwner 抛)。不一致泄漏 assetId 形状。修法:
// resolveAssetOwner 移到 bad-key/assetId 校验前 → invalid/missing/known 无 proof 一律 401。
describe('G2.1 R3-F2 — assets resolveAssetOwner 前置(invalid/missing/known 无 proof 一律 401)', () => {
  let app: Hono<AppEnv>
  beforeEach(() => {
    app = buildAssetsApp()
    resetDecodeGate()
  ...
  it('strict + 无 proof + GET /api/assets/not-a-hash → 401(非 404;assetId 形状校验在 proof 后)', async () => {
    vi.stubEnv('MIVO_SSO_STRICT', '1')
    vi.stubEnv('MIVO_GATEWAY_SECRET', GW)
    const res = await app.request('/api/assets/not-a-hash', { headers: {} })
    expect(res.status).toBe(401) // R3-F2:proof 前置 → 401(返修前 ASSET_ID_RE 先 → 404 泄漏形状)
  })

  it('strict + 无 proof + GET /api/assets/<64hex missing> → 401(非 404;proof 先于 readForOwner)', async () => {
    vi.stubEnv('MIVO_SSO_STRICT', '1')
    vi.stubEnv('MIVO_GATEWAY_SECRET', GW)
    const res = await app.request('/api/assets/' + '0'.repeat(64), { headers: {} })
    expect(res.status).toBe(401)
  })

  it('strict + 无 proof + GET /api/assets/<64hex missing> + garbage token → 401(assets 无 share,gate 不挂;route resolveAssetOwner 前置)', async () => {
    vi.stubEnv('MIVO_SSO_STRICT', '1')
    vi.stubEnv('MIVO_GATEWAY_SECRET', GW)
    const res = await app.request('/api/assets/' + 'a'.repeat(64), {
      headers: { 'x-mivo-share-token': 'garbage' },
    })
    expect(res.status).toBe(401) // assets 不支持 share(gate 不挂);route 内 resolveAssetOwner 在所有 validation 前
  })
})
