...
describe('DP-6R chat per-user — 路由级验收', () => {
  ...
  it('chat PATCH/DELETE per-actor + P2-1 strict-update:非己/不存在 msgId → 404,不借 PATCH create 副本', async () => {
    await setup()
    await req(app, '/api/canvas/c1/chat', { method: 'POST', headers: u('alice'), body: JSON.stringify({ message: { id: 'a-m', text: 'A' } }) })
    await req(app, '/api/canvas/c1/chat', { method: 'POST', headers: u('bob'), body: JSON.stringify({ message: { id: 'b-m', text: 'B' } }) })
    ...
    // P2-1:bob PATCH alice 的 a-m → 404 unknown-message(strict-update,不 create 己方副本,不触 alice 的 a-m)
    ...
    expect((bPatch.body as { error: string }).error).toBe('unknown-message')
    // P2-1:bob PATCH 不存在的 'mX'(带 If-Match)→ 404,不借 PATCH create
    const bPatchMissing = await req(app, '/api/canvas/c1/chat/mX', { method: 'PATCH', headers: { ...u('bob'), 'if-match': '0' }, body: JSON.stringify({ payload: { id: 'mX', text: 'sneak' } }) })
    expect(bPatchMissing.status).toBe(404)
    expect((bPatchMissing.body as { error: string }).error).toBe('unknown-message')
    // alice 的 a-m 仍是 A-edit;bob collection 不新增(仍 [b-m])
    const aList = await req(app, '/api/canvas/c1/chat', { headers: u('alice') })
  ...
  it('P1-2 chat reorder per-actor:独立 orderRevision;同 base 一成一 409;A/B 互不冲突;GET 返 orderRevision', async () => {
    await setup()
    await req(app, '/api/canvas/c1/chat', { method: 'POST', headers: u('alice'), body: JSON.stringify({ message: { id: 'a1', text: 'A1' } }) })
    await req(app, '/api/canvas/c1/chat', { method: 'POST', headers: u('alice'), body: JSON.stringify({ message: { id: 'a2', text: 'A2' } }) })
    await req(app, '/api/canvas/c1/chat', { method: 'POST', headers: u('bob'), body: JSON.stringify({ message: { id: 'b1', text: 'B1' } }) })
    // GET chat 返 orderRevision=0(初始)
    const aGet0 = await req(app, '/api/canvas/c1/chat', { headers: u('alice') })
    expect(aGet0.status).toBe(200)
    expect((aGet0.body as { orderRevision: number }).orderRevision).toBe(0)
    // alice reorder [a1,a2]→[a2,a1] If-Match:0 → 200,返新 contentVersion=1(=新 orderRevision)
    const ro = await req(app, '/api/canvas/c1/reorder', { method: 'POST', headers: { ...u('alice'), 'if-match': '0' }, body: JSON.stringify({ type: 'chat-message', orderedIds: ['a2', 'a1'] }) })
    expect(ro.status).toBe(200)
    expect((ro.body as { reordered: number; contentVersion: number }).contentVersion).toBe(1)
    // GET → orderRevision=1,顺序 [a2,a1]
    const aGet1 = await req(app, '/api/canvas/c1/chat', { headers: u('alice') })
    expect((aGet1.body as { orderRevision: number }).orderRevision).toBe(1)
    expect((aGet1.body as { messages: { id: string }[] }).messages.map((m) => m.id)).toEqual(['a2', 'a1'])
    // 同 base=0 再 reorder → 409(stale,返当前 orderRevision=1)
    const stale = await req(app, '/api/canvas/c1/reorder', { method: 'POST', headers: { ...u('alice'), 'if-match': '0' }, body: JSON.stringify({ type: 'chat-message', orderedIds: ['a1', 'a2'] }) })
    ...
    // bob reorder 自己的 [b1] If-Match:0 → 200(bob 独立 cursor,与 alice 的 bump 互不冲突)
    const bRo = await req(app, '/api/canvas/c1/reorder', { method: 'POST', headers: { ...u('bob'), 'if-match': '0' }, body: JSON.stringify({ type: 'chat-message', orderedIds: ['b1'] }) })
    expect(bRo.status).toBe(200)
    expect((bRo.body as { contentVersion: number }).contentVersion).toBe(1)
    // 匿名 share-link 访客 chat reorder → 401 require-login
    const create = await req(app, '/api/projects/p1/share-links', { method: 'POST', headers: u('alice'), body: JSON.stringify({ permission: 'edit' }) })
    const token = (create.body as { token: string }).token
    const anonRo = await req(app, '/api/canvas/c1/reorder', { method: 'POST', headers: { 'x-mivo-share-token': token, 'if-match': '0' }, body: JSON.stringify({ type: 'chat-message', orderedIds: ['a2', 'a1'] }) })
    expect(anonRo.status).toBe(401)
    expect((anonRo.body as { error: string }).error).toBe('require-login')
    // missing If-Match → 428(base 必填)
    const noBase = await req(app, '/api/canvas/c1/reorder', { method: 'POST', headers: u('alice'), body: JSON.stringify({ type: 'chat-message', orderedIds: ['a2', 'a1'] }) })
    expect(noBase.status).toBe(428)
    expect((noBase.body as { error: string }).error).toBe('precondition-required')
  })
})
