...
| 时刻 | 载体 | 干什么 |
|------|------|--------|
| 每小时整点 | launchd 纯脚本(**不建会话,零 LLM**) | 只查 S0 信号(六项核心流程 error 簇 / nightly 红灯 / 静默失败);命中 → Slack 告警(notify.mjs,未配 webhook 自动降级;附簇详情),等最近班车或人工点开会话 |
| 02:30 每日 | maker 定时会话(主轮) | 会话第一步跑 gate:无活 → 记一行日志秒退;有活 → 全流程 |
| 13:00 每日 | maker 定时会话(补轮) | 只消化上午新进 S0/S1,赶 17:00 部署班车 |

Gap-check 兜底:会话醒来先看距上次日志条目多久,>26h 说明漏跑,拉数窗口自动扩为 `gap + 12h`(7 天保留窗内补得回来)。gate 连续 3 天失败 → Slack 告警"loop 已失明"(降级通道同上)。
...
### T0 · 只告警,不动手
error 突增(单小时 >50)/ persist·迁移类数据完整性信号 / 鉴权异常簇 / debug 系统自身故障。→ Slack 立即告警,等人。
...
6. **交付**:开 PR(证据齐全);T1 达标 → merger worker squash 合并(回复并 resolve 评审线程);否则 Slack 通知留人审。
6.5 **合并后兜底**(借鉴 cindy typecheck-merged):T1 自动合并后立即在本地 main 跑 `npm run preflight`,红 → 自动开 revert PR + Slack T0 告警——防两个绿 PR 语义冲突。
7. **收尾**:无论成败 `worktree remove --force`;更新 State(游标/台账/PR)、Logs 追加一行(含成本);战报发 Slack `#mivo-canvas`(静默日一行摘要)。
...
- **幂等标记**:issue/PR/Slack 消息均嵌隐藏指纹标记,重复触发不重复建档;
...
- **台账互斥锁的接管协议存在不可原子化的三方连环微秒级窗口**(`state.mjs` 陈锁 rename 收尸 + 验尸放回,2026-07-19 四轮审查后定版)。触发需四条件同时满足:①锁已陈死(持有进程崩溃超 TTL 60 分钟);②某旧观察者完成陈旧判定后恰在此处停顿;③停顿期间原尸体被另一接管者收走并换上新锁;④第三方进程在旧观察者恢复后的数个文件操作间隙精确插入。届时错收的新锁无法原位放回,**两个活会话之一的单轮 state 更新会被最后写者覆盖丢失**;saveState 原子写保证 state.json 结构不坏,丢失的更新可由后续 gate 重跑按记录 id 补回(debug log 簇)——人工报单行(intake)无自动重放,丢单需报单人重发。本协议**不承诺"fresh 锁绝不被旧观察者收走"**;若未来要求零丢失,升级路径是 OS 级 CAS(renameat2/RENAME_EXCL)或 flock,本期不做。
