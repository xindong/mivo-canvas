...
| R-7 | **离线 chat 消息 server 模式 boot hydrate 瞬态覆盖(G1-c 硬前置)** | **机制**:离线发的 chat 消息已落 IDB(chatPersistConfig partialize `messagesByScene`)+ enqueue appendChatMessage;若关页面前未 drain,下次启动 server 模式 boot 时 IDB rehydrate 先恢复消息,旋即被 `hydrateFromServer` → `listChatMessages` **wholesale replace** `messagesByScene[sceneId]` 为服务端列表(无 merge-reconcile)整体替换掉;之后 `startPersistWriteQueue` drain 成功落服务端,但 `applyServerRevision` 无 appendChatMessage 回填分支 → store 不回填 → 本会话消息消失,**再次刷新才重现**(无永久数据丢失)。**位置**:`src/lib/persistBoot.ts:239` hydrate 无 merge-reconcile + `src/lib/persistBoot.ts` `applyServerRevision` 无 appendChatMessage 回填分支。**收口路径(G1-c chat 客户端硬前置)**:hydrate 改 merge-reconcile 保留本地未 drain 消息,或 drain 成功后回填 store 二选一;负例验收=离线消息跨 boot 不消失。**可达性**:仅 `?persist=server`/`VITE_MIVO_PERSIST=server` 显式开关可达,生产默认 `local`=no-op(`isLocalPersist` 守卫,`bootPersistWiring` 直接 return)不受影响(2026-07-12 lead 批准 defer-with-doc,G1-c 任务书携带)。 |
...
- **离线 chat 消息 boot hydrate 覆盖(R-7)延 G1-c**:机制确认成立(瞬态可见性,无数据丢失);仅 `?persist=server` 开关可达,生产默认 local 不受影响;收口属 G1-c chat 客户端(与 R-6 同级报 lead,G1-c 任务书携带)。
