...
> 状态：**已裁决（2026-07-10 lead 拍板）**：DP-3 选 (a) 删画布级联软删其 chat collection（一起 restore）；矛盾 2 deleteProject 选 (a) 级联软删其画布（standalone 回落是硬删时代防丢补偿，软删落地后理由消失，且避免孤儿 UI）。软删粒度（不到单 node）、asset 7 天宽限 vs canvas 30 天保留顺序约束维持写法。裁决回填本文件 §3/§7/§8 + plan §3 DP-3 行。
> 日期：2026-07-10。
...
- **DP-3 已裁决（2026-07-10，选 (a)）**：**删画布级联软删对话**（chat collection 随 canvas 一起 `is_deleted=true`，可一起 restore）。理由见 §3。**与 `chatHydration.characterization.test.ts:380` 现状表征直接冲突**（该测试自标"现状（DP-3）"钉死"deleteCanvas 不级联清 chat"）→ 属**有意产品行为变更**，实施 PR 须显式更新表征基线 + 引用本裁决（见 §7 澄清条），非静默改。
- **最大现状冲突（已裁决）**：`deleteProject` 现状是"硬删 project + canvas 回落 standalone"（`projectsSlice.ts:66-100`，#164 钉死）；**裁决（2026-07-10，选 (a)）：改为级联软删其画布**（standalone 回落是硬删时代防丢补偿，软删落地后理由消失，且避免孤儿 UI）→ #164 standalone 回落断言须改，属有意行为变更（§7 澄清条）。见 §6/§7。
...
**已裁决（2026-07-10 lead 拍板）：选 (a) 删画布级联软删其 chat collection，一起 restore。** 下表保留选项/推荐/理由/影响面供追溯，结论即推荐 (a)。
...
| **canvas delete → chat** | **无级联**：`chatHydration.characterization.test.ts:380` 钉死"deleteCanvas 不级联清 chatStore.messagesByScene（跨 store 无订阅）"，`messagesByScene['doomed']` 保留。 | 级联 soft delete chat collection（DP-3 已裁决选 (a)）。 | **行为变 + 表征冲突**（§7 #C2）。迁移：canvas 软删时同步 `is_deleted=true` chat collection。 |
...
> **⚠️ 澄清条（lead 2026-07-10 裁决，防未来误用）**：上述"表征一字不改"铁律约束的是**迁移类 PR**（换数据来源不换行为，如 IDB→服务端、selection 迁 session）。本节 **C1/C2/C5 不属此类**——它们是**有意的产品行为变更**（deleteProject 从 standalone 回落改级联软删；deleteCanvas 从不级联 chat 改级联软删）。处理方式 = **实施该行为变更的 PR 里显式更新表征基线 + PR 说明引用本裁决**，非静默改、非"迁移时顺手改"。迁移类 PR 仍不许动这些表征。
...
| **C2** | #167 `chatHydration.characterization.test.ts` | `:380` "canvasStore.deleteCanvas 不级联清 chatStore.messagesByScene（跨 store 无订阅）"，`:399-400` 断言 `messagesByScene['doomed'].toHaveLength(1)`，注释自标"现状（DP-3）" | 目标（DP-3 已裁决选 (a)）：deleteCanvas 级联 soft delete chat collection | ⚠️ **迁移时须同步迁移表征**：断言从"对话仍在（length 1）"改为"chat collection `is_deleted=true`（软删隐藏，purge 前可 restore）"。注释"现状（DP-3）"改为"DP-3 已定：级联软删"（已裁决，见 §3）。 |
...
> **总结**：⚠️ 碰撞点 = **C1/C5（deleteProject 回落→级联软删）+ C2（deleteCanvas 不级联 chat→级联软删）**。C3/C4 不冲突。DP-3 与 deleteProject 均已裁决选 (a)（2026-07-10，见 §3/§8）；实施这些**有意行为变更**的 PR 须按 §7 澄清条显式更新表征基线 + 引用本裁决，非静默改。

---

## 8. 发现的矛盾与遗留问题（计划 vs 源码；本文件记录，不改源码/计划）

> 边界：只记录，不自行改源码/计划。裁决已回填（2026-07-10 lead 拍板），下标"已裁决"；未裁决项标"待 lead 裁决"。

1. **DP-3 现状表征 vs 目标级联软删**（§7 C2）—— **已裁决（2026-07-10，选 (a)）**：`chatHydration.characterization.test.ts:380` 钉死"deleteCanvas 不级联 chat"且注释自标"现状（DP-3）"——测试作者知道这是 DP-3 待定点先钉了现状。lead 拍板选 (a) 级联软删，**与该表征冲突**，属有意行为变更（§7 澄清条）：实施 PR 显式更新基线 + 引用本裁决，非静默改。
2. **deleteProject 回落 standalone vs 目标级联软删**（§7 C1/C5）—— **已裁决（2026-07-10，选 (a)）**：#164 钉死"回落 standalone"语义。lead 拍板改 (a) 级联软删其画布——理由：standalone 回落是硬删时代的防丢补偿，软删落地后理由消失，且避免"项目软删了、画布还在 standalone 列表"的孤儿 UI。与 #164 冲突，属有意行为变更（§7 澄清条）：实施 PR 显式更新基线 + 引用本裁决。
...
> 本文件不改源码、不改 record-schema/platform-architecture（plan §3 DP-3 行除外，lead 2026-07-10 授权回填）；矛盾记录在 §8，DP-3 + deleteProject 均已裁决选 (a)。
