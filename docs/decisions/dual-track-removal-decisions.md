...
1. **shadow compare 不一致 warn（30 天）= 0**（warn 远传落 JSONL，jq 扫近 30 天文件）
   ...
   echo "$RAW" | jq -s '[.[] | select(.level=="warning" and .source=="Kernel" and ((.message // "") | test("mismatch")))] | length' \
     | { read -r n; [ "$n" = "0" ]; }                                                                  # 计数断言 ==0
   ```
   样例（green）：`0`（rc=0）。fixture dry-run（6 case）：① 2 条 message 含 "mismatch" 的 Kernel warn（窗口内）→ 计数 2 → rc=1 红；② 0 条 mismatch-keyword 但有其他 warn（窗口内）→ 计数 0 → rc=0 绿；③ 空 logdir / 无 *.jsonl → rc=1 红（观测缺失，D4-R7b F2 修复点）；④ 文件全陈旧 mtime>30d → rc=1 红；⑤ 健康态（窗口内有非 mismatch warn）→ rc=0 绿；⑥ 文件 mtime 窗内但内容 receivedAt 陈旧 → rc=1 红（时间戳新鲜断言）。
   新鲜度窗口取 30 天（非 persist 的 4 天）：`RemoteDebugRecord`（`debug-records.ts:17-38`）level 仅 `warning|error`、无 info/heartbeat，`remoteDebugReporter`（`remoteDebugReporter.ts:70-71`）只远传 warn/error——shadow compare 健康态 mismatch=0 时 warn sink 本就稀疏，4 天新鲜度会假红；故窗口=观察窗（30 天），`find -mtime -30` 文件级 + `receivedAt` 记录级双证窗口内有样本。`[ -n "$RAW" ]` 拦空输入防 jq -s slurp `[]` length=0 假绿；`{ read -r n; [ "$n" = "0" ]; }` 同 F1 断言 idiom。`find -mtime -30` 取近 30 天文件、`-exec cat {} +` 无文件不调 cat；`jq -s` 把 JSONL 行 slurp 成数组后 filter。`debugLogger.warn('Kernel', ...)` 经 `remoteDebugReporter` warn level 远传 → POST `/api/mivo/debug-logs` → `appendRemoteDebugRecords`（`debug-records.ts:192`）写 `${date}.jsonl`，`source`/`message`/`level`/`receivedAt` 字段见 `RemoteDebugRecord`（`:17-38`）。
   **r7b 发现（非 F2 范围，flag 给 lead）**：现有 `.message | test("mismatch")` 过滤器是子串匹配，而实际 shadow compare warn 措辞为 `"shadow divergence: record=… field=… expected=… actual=…"`（`useKernelRead.ts:88`），不含 "mismatch" 子串 → 会漏匹配真实 divergence warn（另一类假绿）。F2 仅修新鲜度断言（lead 范围），过滤器措辞是否改 `test("mismatch|divergence")` 留 lead 定夺。
