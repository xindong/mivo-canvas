...
3. **checkpointed rollback drill 通过**（一次性，非 30 天指标）
   ...
   ```
   样例：测试 `pass`；drill 报告 `docs/reports/kernel-rollback-drill-<date>.json`（符 §2.2③ drill report JSON schema，由独立 closeout PR 合入 main）存在且 `jq -e` 校验通过（`roundtripVerdict=="pass"` + 六项 `checks` 全 true）。
