...
| GitHub labels | 创建 `renderer`、`leafer` label（repo 现只有 9 个默认 label，无此二者——dry-run 实测 `gh api .../labels` 过滤 renderer/leafer/kernel 全 `[]`） | `gh api repos/xindong/mivo-canvas/labels --jq '.[].name' \| grep -E 'renderer\|leafer'` 期望非空 |
...
3. **e2e `--renderer=both` 最近 20 merged PR 连续绿**（目标 context 过滤 + 连续性断言，非 `grep -c success` 笼统计）
   ...
   gh pr list -R xindong/mivo-canvas --state merged --limit 25 --json number,mergedAt \
     ...
     | while read pr; do
       ...
       done | awk 'BEGIN{n=0} $0!="SUCCESS"{n++} END{print "non-SUCCESS:", n}'
   ```
   样例（dry-run 2026-07-12 真实数据，PR #216→#198）：`non-SUCCESS: 0`。
   断言：取**最近 20** merged（按 `mergedAt` 排序），任一非 SUCCESS 或 MISSING 即非绿——连续性 = 最近 20 个全绿，**不是**"历史累计 ≥20"。
...
4. **visual-diff (dom vs leafer) 最近 20 merged PR 连续 PASS**（DIFF_THRESHOLD_PERCENT 默认 5%，`scripts/visual-diff.mjs`）
   ...
   ```
   样例（dry-run 2026-07-12）：`SUCCESS`（唯一值；出现非 SUCCESS/MISSING 即非绿）。

5. **leafer 渲染回归 bug P0/P1（30 天内）= 0**
   ```bash
   gh issue list -R xindong/mivo-canvas --label renderer --label leafer --state open --search "P0 OR P1" --json number
   ```
   样例（green）：`[]`（空数组）。**硬前置**：先建 `renderer`/`leafer` label（见上前置表）——dry-run 实测 `--label` 命中不存在的 label **静默返空不报错**，无法区分"无 bug"与"label 缺失"，故 label 验收命令非空是本指标可判定的前提。
...
2. **e2e `--kernel=both` 最近 20 merged PR 连续绿**（双 context 都 SUCCESS；C-switch PR 必先改 CI 矩阵跑 both，见下注）
   ...
   gh pr list -R xindong/mivo-canvas --state merged --limit 25 --json number,mergedAt \
     ...
     | while read pr; do
       ...
       done | awk 'BEGIN{n=0} $1!="SUCCESS" || $2!="SUCCESS" {n++} END{print "non-green PRs:", n}'
   ```
   样例（dry-run 2026-07-12 真实数据，PR #216→#198）：`non-green PRs: 0`（两 context 20/20 SUCCESS）。
   断言：最近 20 merged PR，每个 PR 的 `e2e prod subset (mock upstream)` 与 `e2e kernel gate (new)` 都须 SUCCESS，任一非绿即非绿。

3. **checkpointed rollback drill 通过**（一次性，非 30 天指标）
   ```bash
   npm test -- src/kernel/shadowCompare.test.ts src/kernel/rollbackTrigger.test.ts   # 契约 §4.7 场景 2-4 绿
   ...
   ```
   样例：测试 `pass`；drill 报告 `docs/reports/kernel-rollback-drill-<date>.md` 存在且记 roundtrip 无损结论。

4. **内核回归 bug P0/P1（30 天内）= 0**
   ```bash
   gh issue list -R xindong/mivo-canvas --label kernel --state open --search "P0 OR P1" --json number
   ```
   样例（green）：`[]`。**硬前置**：先建 `kernel` label（§2.1② 前置表实测 repo 无此 label），`--label` 命中不存在 label 静默返空不报错，label 验收非空是本指标可判定前提。
...
- **账号切换 canonical 不撞**：A→B→A→logout→B 全场景 storage key 全属当前 userId。契约 §5.5 验收（FX-6 实施 PR 的契约测试）：
  ```bash
  npm test -- src/kernel/useKernelRead.contract.test.ts   # FX-6 账号切换 e2e 绿
  ```
  样例：`pass`。
...
**注意（防跳阶段，machine-readable 门控；r3 改为外部可核验字段 + guard 走 GitHub API + 自保护如实归 base-side）**：
D PR 不得仅靠"PR 描述显式声明"——描述与事实可脱节，且 r2 的人工填布尔/日期/计数可伪造（R3-P2-4）。
C-switch PR 落盘一个 machine-readable 事件文件 `docs/decisions/kernel-c-switch-event.json`，
**字段全部改为事后可经 GitHub API / 仓库内容核验**（非人工填布尔/计数）：

| 字段 | 值 | 核验依据 |
|---|---|---|
| `cSwitchPrNumber` | C-switch PR 号（如 `216`） | `gh api repos/xindong/mivo-canvas/pulls/<n>` |
| `cSwitchHeadSha` | C-switch PR 的 `merge_commit_sha`（40 字符） | 上同 `.merge_commit_sha` 等于本字段（防填错 PR/SHA） |
| `cSwitchMergedAt` | ISO 8601 合并时刻 | 上同 `.merged_at`——guard **从 API 读**算 30 天，不信人工填日期 |
| `drillReportPath` | rollback drill 报告仓库内相对路径 | `gh api repos/.../contents/<path>` 存在 |
| `drillReportSchema` | 报告 schema 版本（如 `1`） | guard 按版本校验报告字段齐全 |

D PR 的 `scripts/ci/structure-guard.mjs` 加机械检查（每条对应一外部事实，五负例逐一必红）：

1. `gh api repos/xindong/mivo-canvas/pulls/<cSwitchPrNumber>` → `.merged==true` 且 `.merge_commit_sha==<cSwitchHeadSha>`（**伪造 PR 号/假 SHA 必红**）；
2. 从 API 返回的 `.merged_at` 算距 D PR 开工日 ≥ 30 天（**伪造日期必红**——日期来自 API 非人工填）；
3. `gh api repos/xindong/mivo-canvas/contents/<drillReportPath>` 存在 + 报告符 `drillReportSchema` 的 schema（`roundtripVerdict==pass`/`executedAt`/`executor` 齐全）（**缺报告/不符 schema 必红**）；
4. `gh api repos/xindong/mivo-canvas/commits/<cSwitchHeadSha>/check-runs` + 枚举 cSwitch 之后 merged PR，数连续两个 kernel context（`e2e prod subset (mock upstream)` + `e2e kernel gate (new)`）均 `SUCCESS` 的 PR ≥ 20（**从 check-runs API 计数，不信人工填 ciBothGreenRuns；手填 20 必红**）；
...
2. **base-side 定时 workflow（可选加固）**：`.github/workflows/` 加 `schedule` 项，从 **pinned tag**
   （非 main HEAD）拉 `structure-guard.mjs` 跑——D PR 改仓库里的 guard 不影响 pinned 版本执行，
   base-side 红会卡后续 PR；需该 schedule check 进 required context（同 §1.5 两阶段迁移路径）；
3. **org ruleset（终极，需 org owner）**：把"5 条检查"固化为 org 级 ruleset（trunk-guard `18006872`
   现不含此项，member 无权加，见 §1.5 末段）——这是唯一 D PR 改不动的层。
本文如实标注：当前仅第 1 层有效，第 2/3 层标为可选/待 org owner，**不夸大为已自保护**。
...
| 前置 | 动作 | 验收命令 |
|---|---|---|
| 事故登记源（**硬前置**，非"建议补"） | 建 `data-loss` GitHub label + `docs/reports/incident-log.md`（人工登记任何数据丢失事件，记日期/影响/根因/PR）。r2 原文"建议删轨前置补"与"数据丢失=0 硬指标"矛盾，r3 改硬前置 | `gh api repos/xindong/mivo-canvas/labels --jq '.[].name' \| grep -E 'data-loss'` 非空 + `test -f docs/reports/incident-log.md` |
| pm2 日志保留 ≥30 天 | 配 logrotate（daily + 30 份）或 pm2 `--merge --log-date-format` 落盘带时间戳。r2 原命令 `pm2 logs --lines 1000` 只覆盖近若干小时，证不了 30 天零错误 | `ssh "$MIVO_PROD_HOST" 'ls ~/.pm2/logs/mivo-canvas-error.log* \| wc -l'` 期望 ≥30（或日志文件跨度 ≥30 天） |

**硬门控指标（6 项，每条命令已 dry-run；prod-side 命令 form 经本地 PG16/sample log 验证，owner 在生产执行）**：

1. **生产 persist 后端身份 = pg，无 memory 回退（全 30 天）**
   ```bash
   ssh "$MIVO_PROD_HOST" 'printenv MIVO_PERSIST_BACKEND'   # 期望 pg
   ```
   样例（green）：`pg`。`$MIVO_PROD_HOST` 为部署目标机（deploy runbook `/AIGC_Group/mivo-canvas/deploy.sh` 所在 host，owner 填）；`pgConfig.ts:40` kind=pg。

2. **生产 PG 读写命中 > 0 且 30 天单调增长（无"重启清空"假象，每 3 天采样 ≥10 点单调非递减）**
   ```bash
   ssh "$MIVO_PROD_HOST" 'psql "$DATABASE_URL" -c "SELECT count(*) FROM canvas_meta; SELECT count(*) FROM chat_messages;"'
   ssh "$MIVO_PROD_HOST" 'psql "$DATABASE_URL" -c "SELECT date_trunc('"'"'day'"'"', created_at) AS d, count(*) FROM chat_messages WHERE created_at > now()-interval '"'"'30 days'"'"' GROUP BY 1 ORDER BY 1"'
   ```
   样例（fixture dry-run PG16）：行数 `canvas_meta_rows=10, chat_rows=50`；30 天采样 10 点全 `5`（单调非递减）。
   `$DATABASE_URL` 从生产 env 读（`PGPASSWORD` 不内联，走 env 避免密钥入命令历史）。

3. **PG 连接池稳定（30 天 0 次 `ECONNRESET`/pool exhaust/idle timeout）**（扫持久日志，**非** `--lines 1000`）
   ```bash
   ssh "$MIVO_PROD_HOST" 'cat ~/.pm2/logs/mivo-canvas-error.log ~/.pm2/logs/mivo-canvas-error.log.* 2>/dev/null | grep -ciE "ECONNRESET|pool.*exhaust|idle.*timeout"'
   ```
   样例（green）：`0`（sample log dry-run：含 `ECONNRESET` + `pool exhausted` 各 1 → 计数 `2`，证 grep 模式可判）。
...
4. **backup+restore drill 窗口内 ≥ 1 次成功（业务行 + asset 可恢复）**——drill 报告路径入删轨 PR 的 machine-readable 事件（见 §2.2③ 末 D 门控注意块，persist drill 报告同构复用该事件文件字段结构，R3-P2-4 改可核验字段）。P0.2 drill 已验。

5. **数据丢失事故 = 0（30 天）**（事故登记源为硬前置，见上前置表）
   ```bash
   gh issue list -R xindong/mivo-canvas --label data-loss --state all --json number,title   # 期望 []
   test ! -s docs/reports/incident-log.md || ! grep -q '^## ' docs/reports/incident-log.md   # 期望无事故条目（文件可空但无 ## 事故段）
   ```
   样例（green）：`[]`（dry-run `--search "data loss 数据丢失"` 真实返 `[]`；`data-loss` label 当前不存在，**硬前置**先建）。r2 原文"无系统化自动源、建议补"与"=0 硬指标"矛盾，r3 改：事故登记源（label + 日志）是硬前置，本指标查该源。
...
6. **`backend.contract.dual` 连续绿（pg-suite required，双后端契约守 memory 兼容直到 D PR 删 memory 半）**
   ...
   ```
   样例（dry-run 2026-07-12 真实数据）：`non-SUCCESS: 4`——pg-suite 2026-07-11（#210）才翻 required，最近 20 merged 含翻 required 前的 PR 故非绿（**命令正确反映当前未达**）。删轨门槛：翻 required 后连续 20 PR 全绿 → `non-SUCCESS: 0`，当前未达（符合 persist 删轨最晚预期）。
...
| kernel 跳阶段（C 未完成开 D） | split-brain、用户数据丢失 | D PR machine-readable 事件 `kernel-c-switch-event.json` + structure-guard 走 GitHub API 校验 5 条外部事实（PR merged+SHA 匹配 / mergedAt≥30 天 / drill 报告符 schema / check-runs 连续 ≥20 / `kernelMode.ts` 不含 `'legacy'`）；自保护如实归 base-side（审查纪律 + 可选 pinned-tag workflow + org ruleset），非"历史断言自保护"（§2.2③） |
