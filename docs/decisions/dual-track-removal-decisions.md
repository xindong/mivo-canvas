...
| PG backend 已实现 | `server/persist/pgBackend.ts`、`server/persist/pgPermissionBackend.ts`、`server/persist/pgConfig.ts`、`server/persist/migrations.ts`(Kysely runner apply DDL) |
| swap 注入点已钉（不改路由/契约） | `server/persist/backend.ts:5-10` 注释："server/app.ts 注入点从 InMemoryPersistBackend 换 PgPersistBackend,路由 handler 零改动" |
...
> **诚实边界（r10）**：本观测完整性前置层只保证「已落盘数据的读取完整」（枚举/可读/格式/新鲜度四步，定义见下 §2.2② 指标1 代码块首注释模板块，persist §3 指标3 同构引用）；**不保证「该落盘的都落盘」**——divergence warn 若恰在 BFF 短暂不可达期间产生，`remoteDebugReporter` 发送失败静默丢批次则根本不落 JSONL，本层读不到也无从报红。「该落盘的都落盘」由 §2.2③ 删轨前置清单 FX-7（remoteDebugReporter 传输加固：发送失败重试 + 持久化待发队列 + 丢弃计数可观测）保证；D-4 观测完整性层不夸大覆盖传输层。

1. **shadow compare 不一致 warn（30 天）= 0**（warn 远传落 JSONL，jq 扫近 30 天文件）
   ```bash
   # ══ 观测完整性前置层（r9 统一模式 — 终结"部分观测缺失假绿"族）══════════════════════
   # 同构四步，所有硬门控日志/观测读取命令套用（本块为定义处，persist §3 指标3 同构引用）：
   #   ① 枚举与读取分离 — find/通配列表先落变量，不直接 -exec cat 喂下游管道
   #   ② 逐文件读取验 rc — cat/gunzip -c 失败 → 显式 RED 输出文件路径 + exit 1；
   #                      rc 在管道外判定（命令替换 $(...) 后外显式 || ），禁 2>/dev/null 在管道内吞 rc
   #   ③ 格式完整性 — JSONL 逐行喂 jq -e . 解析，截断/半残行即红（禁 2>/dev/null 吞 jq 错误，解析诊断须可见）；
   #                   非 JSONL 文本日志由 ② 的 cat rc + .gz 的 gunzip -t 覆盖可读性
   #   ④ 既有断言不回退 — 存在性/非空/新鲜度/过滤器/count 语义全保留
   # ═════════════════════════════════════════════════════════════════════════════════
   # 生产 BFF 机：jq 扫近 30 天 debug-logs JSONL，数 source=Kernel Shadow 且 message 含 shadow divergence 的 warn
   # r7b F2 新鲜度 + r7b (a) 过滤器对齐真实措辞 保留；r9 再补：原 find -exec cat {} + 2>/dev/null 吞不可读
   #   JSONL（chmod 000/permission）→ 该文件数据静默丢，若含唯一 shadow divergence warn → count=0 假绿（lead (C) 统一修）。
   LOGDIR="${MIVO_DEBUG_LOG_DIR:-data/debug-logs}"
   CUTOFF=$(date -u -v-30d +%Y-%m-%d 2>/dev/null || date -u -d '30 days ago' +%Y-%m-%d)
   FILES=$(find "$LOGDIR" -name '*.jsonl' -mtime -30 2>/dev/null)                                    # ① 枚举落变量
   [ -n "$FILES" ] || { echo "RED: kernel warn sink 缺失（近 30 天无 *.jsonl）"; exit 1; }              # ④ 存在性
   TMP=$(mktemp); trap 'rm -f "$TMP"' EXIT
   while IFS= read -r f; do
     [ -f "$f" ] || continue
     cat "$f" >> "$TMP" || { echo "RED: kernel warn sink 不可读: $f"; rm -f "$TMP"; exit 1; }         # ② 逐文件验 rc（管道外）
     printf '\n' >> "$TMP"                                                                         # 文件间换行防 cat 无尾换行致行合并
   done <<< "$FILES"
   [ -s "$TMP" ] || { echo "RED: kernel warn sink 全空（*.jsonl 存在但内容全空）"; exit 1; }              # ④ 非空
   while IFS= read -r line; do
     [ -z "$line" ] && continue
     printf '%s' "$line" | jq -e . >/dev/null || { echo "RED: kernel warn sink JSONL 行不可解析（截断/半残）: $line"; rm -f "$TMP"; exit 1; }  # ③ 格式完整性
   done < "$TMP"
   jq -se --arg c "$CUTOFF" '([.[] | .receivedAt // empty] | sort | last // "") >= $c' < "$TMP" >/dev/null \
     || { echo "RED: kernel warn sink 陈旧（最新 receivedAt 不在 30 天窗内）"; exit 1; }                # ④ 新鲜度
   jq -s '[.[] | select(.level=="warning" and .source=="Kernel Shadow" and ((.message // "") | test("shadow divergence")))] | length' < "$TMP" \
     | { read -r n; [ "$n" = "0" ]; }                                                              # ④ 过滤器 + 计数==0
   ```
   样例（green）：`0`（rc=0）。fixture dry-run（6 case，真实措辞 source=`Kernel Shadow` + message=`shadow divergence`，r7b (a) 对齐 `useKernelRead.ts:36/90`）：① 1 条真实 shadow divergence warn → 计数 1 → rc=1 红（**r7b (a) 修复点**：旧过滤器 `source=="Kernel"` + `test("mismatch")` 双漏匹配真实记录 → 计数 0 假绿）；② 2 条真实 shadow divergence warn → 计数 2 → rc=1 红；③ 0 shadow divergence 但有其他 Kernel 域 warn（source=`Kernel` "未知 mode"）→ 计数 0 → rc=0 绿；④ 空 logdir / 无 *.jsonl → rc=1 红（观测缺失，r7b F2 修复点）；⑤ 文件全陈旧 mtime>30d → rc=1 红；⑥ 文件 mtime 窗内但 receivedAt 陈旧 → rc=1 红（时间戳新鲜断言）。**r9 新增 ⑦⑧**（lead (C) 统一修"部分观测缺失假绿"族，fixture 真跑证据见 commit）：⑦ 唯一 shadow divergence warn 落在 chmod 000 不可读文件、另一文件 clean 可读 → 旧 `find -exec cat {} + 2>/dev/null` 吞 cat permission-denied → 仅读 clean 文件 → count=0 **假绿 rc=0**；新命令逐文件 `cat "$f" >> "$TMP" || RED` 验 rc（管道外）→ **rc=1 红 + 文件路径**（r9 修复点）。⑧ 截断半残 JSONL 行（`...,"mes` 无结尾）→ 旧 `jq -s` slurp 整批失败给"陈旧"误导红；新命令逐行 `jq -e .` 解析 → **rc=1 红 + 精确行内容**（r9 修复点，诊断可见）。
   新鲜度窗口取 30 天（非 persist 的 4 天）：`RemoteDebugRecord`（`debug-records.ts:17-38`）level 仅 `warning|error`、无 info/heartbeat，`remoteDebugReporter`（`remoteDebugReporter.ts:70-71`）只远传 warn/error——shadow compare 健康态 mismatch=0 时 warn sink 本就稀疏，4 天新鲜度会假红；故窗口=观察窗（30 天），`find -mtime -30` 文件级 + `receivedAt` 记录级双证窗口内有样本。`[ -n "$FILES" ]` 拦空枚举防 jq -s slurp `[]` length=0 假绿（r9：枚举与读取分离，`FILES` 落变量替旧 `-exec cat` 内联喂下游）；`{ read -r n; [ "$n" = "0" ]; }` 同 F1 断言 idiom。`find -mtime -30` 取近 30 天文件；r9 逐文件 `cat "$f" >> "$TMP" || RED` 验 rc（管道外，替旧 `-exec cat {} + 2>/dev/null` 吞不可读文件假绿），逐行 `jq -e .` 验 JSONL 完整性（替旧 `jq -s` 整批失败给"陈旧"误导红），`jq -s` 在完整性已验后 slurp 成数组做 filter/count。`debugLogger.warn('Kernel Shadow', ...)`（SOURCE='Kernel Shadow' 见 `useKernelRead.ts:36`，r7b (a) 修正：原文档写 `'Kernel'` 与真实 SOURCE 不符）经 `remoteDebugReporter` warn level 远传 → POST `/api/mivo/debug-logs` → `appendRemoteDebugRecords`（`debug-records.ts:192`）写 `${date}.jsonl`，`source`/`message`/`level`/`receivedAt` 字段见 `RemoteDebugRecord`（`:17-38`）。
   **r7b (a) 已修（lead 裁定）**：F2 commit `bb03b48` 仅修新鲜度断言时发现过滤器**双缺陷**——`.source=="Kernel"`（实际 `SOURCE='Kernel Shadow'`，`useKernelRead.ts:36`）+ `test("mismatch")`（实际 message 措辞 `"shadow divergence: record=…"`，`useKernelRead.ts:90`）——两个条件都漏匹配真实 shadow compare warn → 真实分歧计 0 假绿（比新鲜度更致命：新鲜度只防"无观测假绿"，过滤器错导致"有真实 divergence 但计数 0 假绿"）。lead (a) 裁定修：过滤器对齐真实措辞 `.source=="Kernel Shadow"` + `test("shadow divergence")`（精确前缀，零误匹配——shadow compare 是 Kernel 域唯一 emit "shadow divergence" 的源；若未来措辞漂移应在改措辞 PR 同步改过滤器，fail visibly 而非宽泛 `test("divergence|mismatch")` 掩盖）。dry-run 负例（lead 要求）：1 条真实 shadow divergence warn（source=Kernel Shadow，message=shadow divergence: record=…）→ 计数 1 → rc=1 红（旧过滤器假绿）；source=Kernel（非 Kernel Shadow）即使 message 含 shadow divergence 也不计数（source 过滤器精确隔离 shadow compare 源）。
   **r9 已修（lead (C) 统一修）**：每轮 Greptile 重审冒同族新 P1（r7b 空输入→r8 .gz→本轮 line 429 不可读 JSONL 被 `2>/dev/null` 静默吞）。lead 裁定不再逐条修，给所有硬门控日志/观测读取命令做统一**观测完整性前置层**（同构四步，定义见本块首注释模板块；persist §3 指标3 同构引用）：①枚举与读取分离（`FILES` 落变量）②逐文件读取验 rc（`cat ... || RED+路径`，rc 管道外，禁 `2>/dev/null` 在管道内吞）③格式完整性（JSONL 逐行 `jq -e .`，截断/半残即红，诊断可见）④既有断言不回退。终结"部分观测缺失假绿"族——旧 `find -exec cat {} + 2>/dev/null` 吞 chmod 000 文件假绿（fixture ⑦ 精确复现 old GREEN rc=0 vs new RED rc=1）。dry-run 8 case 全过（r7b(a) 6 case 回归 + r9 ⑦⑧ 新增），证据见 commit。
...
- **生产 kernel 身份**：切默认后 `?kernel=` 缺省解析 new；`?kernel=legacy` 仅受控 rollback drill 产生（非用户流量）。
  ```bash
  psql "$DATABASE_URL" -tA -c "SELECT count(*) FROM mode_hits WHERE track='kernel' AND mode='legacy' AND ts > now()-interval '30 days'" | { read -r n; [ -n "$n" ]; }
  ```
  样例（green，C 后）：≈ drill 次数（1-2，来自受控 drill 时段）；若远超 drill 计数 = 有用户回退，须查。不卡删轨，作旁证监控。断言（r7b (b) 改 Greptile F1-class）：原 `psql -c "SELECT count(*)"` 只打印 count、rc 恒 0——psql 连接失败/输出空也 rc=0（观测缺失假绿，监控数字不可信会误导删轨判断）。改 `-tA` 取净值 + `{ read -r n; [ -n "$n" ]; }`：n 非空（psql 成功输出计数）rc=0 绿；n 空（psql 失败/观测缺失）rc=1 红。**不用 `[ "$n" = "0" ]`**：本监控项 green 语义是 `≈ drill 次数（1-2）` 非 0（count=0 反意味 drill 未跑、亦须查），`==0` 会判正常 drill 态红、违背 lead "口径不变"——故断言只验"观测存在"（数字可信），数值本身（1-2 vs 远超）仍人工判断（监控非门控）。dry-run 正例 `echo 1 | { read -r n; [ -n "$n" ]; }`→rc=0 绿（drill 计数）；`echo 0 | { read -r n; [ -n "$n" ]; }`→rc=0 绿（0 是有效观测、须查但 psql 成功）；负例 `printf '' | { read -r n; [ -n "$n" ]; }`→rc=1 红（psql 失败/观测缺失）。
...
**删轨前置清单（r10 补 FX-7，machine-readable）**：
- [ ] **FX-7** `remoteDebugReporter` 传输加固已合入 main（发送失败重试 + 持久化待发队列 + 丢弃计数可观测——保证 divergence warn 不因 BFF 短暂不可达而静默丢失、不落 JSONL 致 §2.2② 观测完整性层读不到也无从报红）；核验：`gh api repos/xindong/mivo-canvas/pulls/<FX-7-PR-number> --jq '.merged'` 返回 `true` 且 `grep -qE 'retry|maxRetries|retryCount|backoff|pendingQueue|persistedQueue|outbox|dropped|dropCount' src/store/remoteDebugReporter.ts` rc=0；FX-7 未合入时 grep 断言必红——当前 main 上 `flushRemoteDebugEntries` 发送失败走 catch 静默吞错（批次已 dequeue 即永久丢）+ `queue` 纯内存不持久化 + 无丢弃计数，r10 dry-run 实测 grep rc=1（9 符号全 absent），证据见 commit。
...
3. **PG 连接池稳定（30 天 0 次 `ECONNRESET`/pool exhaust/idle timeout）**（扫持久日志，**非** `--lines 1000`）
   ...
   ssh "$MIVO_PROD_HOST" '
     ...
     # ── r9 观测完整性前置层（统一模式，定义见 §2.2 指标1；同构 ①②③④）──
     #  r7b F4 存在性/非空 + r8 .gz gunzip -t 预验 保留；r9 再补：原管道内 cat/gunzip -c 2>/dev/null 的 rc
     #  被末段 grep/read 吞 → 文件不可读（chmod 000）静默丢数据 → 计数可 0 假绿。改逐文件管道外验 rc + 临时文件合流。
     # ① 枚举与读取分离：存在性/非空断言（r7b F4 不回退）
     NE=0; for f in "$BASE" "$BASE".*; do [ -f "$f" ] && [ -s "$f" ] && NE=1; done
     [ "$NE" -eq 1 ] || { echo "RED: pm2 error log 全缺失或全空（观测缺失）"; exit 1; }
     # ② 逐文件读取验 rc（管道外）：.gz 先 gunzip -t 验可解压（不可读/损坏即红，禁 2>/dev/null 吞 rc，诊断须可见）
     for g in "$BASE".*.gz; do [ -f "$g" ] || continue; gunzip -t "$g" || { echo "RED: gz 不可读/损坏: $g"; exit 1; }; done
     # ②+③ 逐文件读取合流落临时文件（cat/gunzip -c 失败显式 RED+路径，rc 管道外判定，不依赖下游 grep 吞 rc）
     TMP=$(mktemp)
     for f in "$BASE" "$BASE".*; do
       [ -f "$f" ] || continue
       case "$f" in
         *.gz) gunzip -c "$f" >> "$TMP" || { echo "RED: gunzip 读取失败: $f"; rm -f "$TMP"; exit 1; } ;;
         *)    cat "$f"        >> "$TMP" || { echo "RED: cat 读取失败: $f"; rm -f "$TMP"; exit 1; } ;;
       esac
     done
     [ -s "$TMP" ] || { echo "RED: pm2 error log 合流后全空（观测缺失）"; rm -f "$TMP"; exit 1; }       # ④ 非空断言不回退
     # ④ 既有断言不回退：grep -ciE 计数 ==0（ECONNRESET/pool exhaust/idle timeout）
     grep -ciE "ECONNRESET|pool.*exhaust|idle.*timeout" "$TMP" | { read -r n; [ "$n" = "0" ]; }
     rc=$?; rm -f "$TMP"; exit $rc
   '
   ```
   样例：0 命中 → rc=0 绿；≥1 命中 → rc=1 红。dry-run sample log（5 case）：① 含 `ECONNRESET`+`pool exhausted`+`idle timeout` 各 1（count=3）→ rc=1 红；② 全 clean 但文件非空（有其他非致命 error）→ count=0 → rc=0 绿；③ error log 文件缺失（pm2 未起/log 目录被删）→ rc=1 红（D4-R7b F4 修复点）；④ error log 存在但全空 → rc=1 红（观测缺失）；⑤ base 空（刚重启）但 rotated clean 非空 → rc=0 绿。**r9 新增 ⑥**（lead (C) 统一修，fixture 真跑证据见 commit）：⑥ 唯一致命 error（`idle timeout`）落在 chmod 000 不可读 base 文件、rotated clean 可读 → 旧管道内 `cat "$BASE" 2>/dev/null` 吞 cat permission-denied → grep 仅扫 rotated clean → count=0 **假绿 rc=0**；新命令逐文件 `cat "$f" >> "$TMP" || RED` 验 rc（管道外，临时文件合流替旧 `{ cat; case } | grep` 管道吞 rc）→ **rc=1 红 + 文件路径**（r9 修复点）。
   ...
   **r9 已修（lead (C) 统一修）**：r8 修了 `.gz` 二进制喂 grep 假绿，但本轮 Greptile 发现同族下一站——管道内 `cat/gunzip -c 2>/dev/null` 的 rc 被末段 `grep/read` 吞，chmod 000 不可读文件静默丢数据（错误仅在不可读文件时 count=0 假绿）。lead 裁定统一修：套用 §2.2 指标1 定义的**观测完整性前置层**同构四步——①枚举落变量（NE 存在性/非空）②逐文件 `gunzip -t` + `cat/gunzip -c >> "$TMP" || RED+路径` 验 rc 管道外（禁 `2>/dev/null` 在管道内吞，`gunzip -t` 诊断须可见）③文本日志由 cat rc + gunzip -t 覆盖可读性 ④既有断言不回退。临时文件合流替旧 `{ cat; case } | grep` 管道（避免 rc 被末段 grep 吞），`grep -ciE` 在 TMP 上计数 ==0。dry-run 7 case 全过（r8 5 case 回归 + r9 ⑥ 不可读新增），fixture 真跑证据见 commit。
