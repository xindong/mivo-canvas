...
1. **shadow compare 不一致 warn（30 天）= 0**（warn 远传落 JSONL，jq 扫近 30 天文件）
   ...
   # r7b Greptile F2：对齐 persist §3 指标 1（D4-R5-2）新鲜度断言——原命令空 logdir/无 JSONL sink
   #   时 jq -s slurp 空输入 → length=0 当 PASS（观测缺失假绿）；现先断言观测源存在+非空+窗口内有样本。
   LOGDIR="${MIVO_DEBUG_LOG_DIR:-data/debug-logs}"
   CUTOFF=$(date -u -v-30d +%Y-%m-%d 2>/dev/null || date -u -d '30 days ago' +%Y-%m-%d)
   RAW=$(find "$LOGDIR" -name '*.jsonl' -mtime -30 -exec cat {} + 2>/dev/null)
   [ -n "$RAW" ] || { echo "RED: kernel warn sink 缺失（近 30 天无 *.jsonl 或全空）"; exit 1; }   # 文件存在 + 非空
   echo "$RAW" | jq -se --arg c "$CUTOFF" '([.[] | .receivedAt // empty] | sort | last // "") >= $c' >/dev/null \
     || { echo "RED: kernel warn sink 陈旧（最新 receivedAt 不在 30 天窗内）"; exit 1; }            # 时间戳新鲜
   echo "$RAW" | jq -s '[.[] | select(.level=="warning" and .source=="Kernel Shadow" and ((.message // "") | test("shadow divergence")))] | length' \
     | { read -r n; [ "$n" = "0" ]; }                                                                  # 计数断言 ==0
   ```
   样例（green）：`0`（rc=0）。fixture dry-run（6 case，真实措辞 source=`Kernel Shadow` + message=`shadow divergence`，r7b (a) 对齐 `useKernelRead.ts:36/90`）：① 1 条真实 shadow divergence warn → 计数 1 → rc=1 红（**r7b (a) 修复点**：旧过滤器 `source=="Kernel"` + `test("mismatch")` 双漏匹配真实记录 → 计数 0 假绿）；② 2 条真实 shadow divergence warn → 计数 2 → rc=1 红；③ 0 shadow divergence 但有其他 Kernel 域 warn（source=`Kernel` "未知 mode"）→ 计数 0 → rc=0 绿；④ 空 logdir / 无 *.jsonl → rc=1 红（观测缺失，r7b F2 修复点）；⑤ 文件全陈旧 mtime>30d → rc=1 红；⑥ 文件 mtime 窗内但 receivedAt 陈旧 → rc=1 红（时间戳新鲜断言）。
   新鲜度窗口取 30 天（非 persist 的 4 天）：`RemoteDebugRecord`（`debug-records.ts:17-38`）level 仅 `warning|error`、无 info/heartbeat，`remoteDebugReporter`（`remoteDebugReporter.ts:70-71`）只远传 warn/error——shadow compare 健康态 mismatch=0 时 warn sink 本就稀疏，4 天新鲜度会假红；故窗口=观察窗（30 天），`find -mtime -30` 文件级 + `receivedAt` 记录级双证窗口内有样本。`[ -n "$RAW" ]` 拦空输入防 jq -s slurp `[]` length=0 假绿；`{ read -r n; [ "$n" = "0" ]; }` 同 F1 断言 idiom。`find -mtime -30` 取近 30 天文件、`-exec cat {} +` 无文件不调 cat；`jq -s` 把 JSONL 行 slurp 成数组后 filter。`debugLogger.warn('Kernel Shadow', ...)`（SOURCE='Kernel Shadow' 见 `useKernelRead.ts:36`，r7b (a) 修正：原文档写 `'Kernel'` 与真实 SOURCE 不符）经 `remoteDebugReporter` warn level 远传 → POST `/api/mivo/debug-logs` → `appendRemoteDebugRecords`（`debug-records.ts:192`）写 `${date}.jsonl`，`source`/`message`/`level`/`receivedAt` 字段见 `RemoteDebugRecord`（`:17-38`）。
