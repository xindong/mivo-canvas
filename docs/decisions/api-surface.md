...
**返修 #5:canvas metaRevision 与 contentVersion 分名**——`CanvasMeta.metaRevision`(canvas meta record envelope.revision,PUT /api/canvas/:id 的 If-Match base)+ `CanvasMeta.contentVersion`(content 版本 counter,backend 在**共享**子资源 node/edge/anchor 写入时 bump,存 canvas meta payload.contentVersion)。子资源写入不 bump metaRevision;client 据此探测 content 是否变化,与 metaRevision 独立。**DP-6R(P1-2)**:chat-message per-actor 私有,写入/reorder **不 bump** 共享 contentVersion;chat reorder 走独立 per-actor×canvas `orderRevision`(见 §4.2.3 ListChatMessagesResponse)。
...
> 画布 = record 集合(nodes/edges/anchors + meta),每 record 独立 revision(§13.5 节点级合并)。chat 是 ~~per-canvas 独立 collection(DP-6,随画布分享)~~ → **DP-6R:per-actor 私有 collection**(ownerId=actor,record-schema §5),不随画布分享、成员互不可见;chat-collection 仍 per-canvas under canvas owner(仅 liveness,随 canvas 生命周期级联 FX-7)。**返修 #2**:chat-collection 是独立 envelope record(type='chat-collection', canvas_id=canvasId,ownerId=canvas owner),canvas 软删时标 collection record;message 不软删(硬删)。chat-message ownerId=actor(PK=(actor,'chat-message',msgId));匿名 share-link 访客 chat 读写一律 401 require-login。
...
| `POST /api/canvas/:id/reorder` | JSON `{type:'node'|'edge'|'anchor'|'chat-message', orderedIds:string[]}` + **`If-Match`(base,必填 F5)** | 200 `{reordered: number, contentVersion: Revision}`(F5:If-Match 必填——缺→428,invalid→400,stale→409 两并发一成一 409;返修 #6 重排持久化 orderKey,重启顺序一致);orderedIds 须与 live set 全等且唯一(缺/多/重复→400)。**DP-6R P1-2**:`type='chat-message'` 时 base = per-actor×canvas **orderRevision**(非共享 contentVersion;见 §4.2.3 GET /chat 返 `orderRevision`),response `contentVersion` = bump 后的新 orderRevision;node/edge/anchor base 仍 = 共享 contentVersion。chat reorder 不 bump 共享 cv;A/B 不同 actor 各自独立 cursor 互不冲突;node 写 bump 共享 cv 不使 chat reorder 误 409 |
...
| 方法+路径 | 请求 | 响应 |
|---|---|---|
| `GET /api/canvas/:id/chat` | — | 200 `{messages: RecordEntry[], orderRevision: Revision}`(ORDER BY orderKey #6;**DP-6R P1-2**:`orderRevision` = per-actor×canvas chat collection cursor,作下次 chat reorder 的 If-Match base);404 unknown-canvas;**401 require-login**(匿名 share-link 访客) |
| `POST /api/canvas/:id/chat` | JSON `{message: ChatMessage}` + `Idempotency-Key` | 201 `{id, revision}`(**唯一 create 入口**;owner=actor,idempotent 同 message.id+actor→200);404(canvas 须未软删);413;**401 require-login**(匿名访客) |
| `PATCH /api/canvas/:id/chat/:msgId` | JSON `{payload: ChatMessage}` + `If-Match`(必填 #4) | 200 `{id, revision}`;409 revision-conflict;**404 unknown-message**(actor bucket 无此 msgId/已删——**DP-6R P2-1 strict-update:不许借 PATCH create 己方副本**,POST 是唯一 create 入口;带 If-Match 也不 create);428 缺 base;413;401 require-login(匿名访客) |
| `DELETE /api/canvas/:id/chat/:msgId` | — | 204(**真硬删 #2**,只删 actor 自己的);404 unknown-message(非己 msgId 不触他人);401 require-login(匿名访客) |

> chat-collection envelope:`type='chat-collection'`, `canvas_id=:id`, `ownerId=canvas owner`, `payload={}`;随 canvas soft delete 级联标(#2),仅 per-canvas liveness,不含 per-actor 状态。chat-message envelope:`ownerId=actor`,PK=(actor,'chat-message',msgId)(per-actor namespace)。ChatMessage 17 字段(chatStore.ts:65-83),payload 不透明整存;ChatMessage.status 是消息状态(合法,不在 #13 白名单拒收范围——#13 仅针对 NodeRecord/EdgeRecord/AnchorRecord)。chat 写入/PATCH/DELETE/reorder **不 bump 共享 canvas contentVersion**;reorder 走独立 orderRevision(见下 §4.2.2 reorder 行注)。
