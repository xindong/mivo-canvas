...
- **canvas metaRevision vs contentVersion 分名**(#5):`metaRevision`=canvas meta record envelope.revision(PUT /api/canvas/:id 的 If-Match base);`contentVersion`=canvas meta payload.contentVersion,backend 在**共享**子资源(node/edge/anchor)写入时 bump,**不动 metaRevision**。bump 用原子 SQL:`jsonb_set(payload,'{contentVersion}',to_jsonb(COALESCE((payload->>'contentVersion')::int,0)+1))`——单语句原子,并发子资源写无 lost-update。**DP-6R(P1-2)**:chat-message per-actor 私有,写入/reorder **不 bump** 此共享 contentVersion;chat reorder 的乐观锁走独立 cursor——`chat_order_revisions(actor_id, canvas_id, revision)` 表(migration 004,PK=(actor_id,canvas_id)),事务内原子 compare+bump:`INSERT (revision=1) ON CONFLICT (actor,canvas) DO UPDATE SET revision=revision+1 WHERE revision=base RETURNING revision`(行不存在即视作 0,首条 reorder INSERT;行存在且 base!==current → 0 rows → 409 返当前 revision)。同 base 两并发一成一败;A/B 不同 actor 各自独立行互不冲突;node 写 bump 共享 cv 不触 chat cursor → 不误 409。
