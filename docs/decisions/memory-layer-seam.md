...
2. chat 在 **document 域**（per-canvas 共享，随画布分享对成员可见，platform §13.5）；记忆在 **user 域**（per-user 独享，D4 owner-独享）。**两者不同域**——chat 是共享对话流，记忆是个人派生缓存。
...
### 4.2 user-state namespace 挂接点（推荐落法）

`shared/persist-contract.ts:291`：
```ts
export const USER_STATE_KEY_NAMESPACES = ['canvas:', 'recent:', 'pref:', 'panel:'] as const
```
天然挂接点：N3 加 `'memory:'` namespace（image/canvas/project/personal 四层用二级前缀区分，如 `memory:image:` / `memory:canvas:` / `memory:project:` / `memory:personal:`）。namespace 校验机制（`isUserStateKeyNamespaceAllowed`，`shared/persist-contract.ts:293`）已就位，加一条 prefix 即可，零新基础设施。

`server/routes/userState.ts`（/api/user-state 路由）：per-owner KV 永不 share + resolveActor owner seam + 幂等 + If-Match + 敏感字段扫描全已落地（文件头注释 N1-N10）。记忆 KV 复用此路由 = 零新 authz/幂等/校验代码。
...
| # | 决策点 | 选项 | 倾向 | 依据 |
|---|------|------|------|------|
| D-mem-1 | 记忆落 scope | (a) 骑 user 域 `memory:` namespace (b) 新增 `PersistScope='memory'` 独立 scope (c) 新建 `/api/memory` 路由+独立表 | **(a)** | §3 判断记忆=user 域语义；复用 `/api/user-state` 零新基础设施（§4.2）；(b)(c) 是 over-engineering 除非共享记忆立项 |
| D-mem-2 | 四层记忆的 key 形状 | (a) `memory:image:<canvasId>:<nodeId>` / `memory:canvas:<canvasId>` / `memory:project:<projectId>` / `memory:personal:<key>` (b) 拆四个 namespace | **(a)** | 单一 `memory:` prefix + 二级前缀分层，namespace 校验机制（§4.2）已支持 prefix 匹配；拆四 namespace 增加冻结面 |
