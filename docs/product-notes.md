...
- New user-facing features must follow the [Development Feedback Rule](development-logging.md): emit Debug Log entries through `debugLogger` for diagnosable state changes, workflow results, skipped paths, and failures, and use `toastFeedback` for short user-visible success, info, warning, and error outcomes.
...
- Canvas selection shortcuts follow mature-tool conventions: `Cmd/Ctrl+A` selects all visible objects (same eligibility as marquee selection), `Cmd/Ctrl+X` cuts the selection to the Mivo clipboard using the same lock and Section-content rules as delete, and `Cmd/Ctrl+G` / `Cmd/Ctrl+Shift+G` group and ungroup the selection. Holding `Alt/Option` while dragging a corner resize handle resizes single nodes and multi-selections around their center; centered resize intentionally skips object snapping so the anchor stays exact.
...
- The Markup shape tool set keeps Arrow (`A`), Line (`L`), Rectangle (`R`), and Ellipse (`O`) behind one hoverable Draw toolbar button so future shapes can expand the same second-level tool tray. Brush (`P`) is a first-class dock tool following FigJam's marker convention, and Markup note (`N`) remains separate from shape drawing.
- Brush follows the FigJam/Excalidraw drawing model: the tool stays active after each stroke for continuous drawing (Esc or `V` exits), and an options bar next to the dock picks marker vs highlighter vs eraser, three stroke widths (2/4/8), and stroke color from the shared markup palette (default black). The last-used brush style is remembered in local persistence. Each brush mode shows its own color-tinted pen cursor on the canvas. Eraser mode (`E`, FigJam-style) removes whole brush strokes touched by the eraser circle while dragging — one drag is one undo step — and leaves other node types alone; `P` always returns to drawing with the marker. Strokes render as pressure-aware variable-width filled paths through the MIT-licensed `perfect-freehand` library with Excalidraw's tuning; real stylus pressure is recorded per point for pen input while mouse strokes use velocity-simulated pressure. Highlighter strokes are a wider, uniform-width, semi-transparent variant stored as `markupBrushKind`. Dashed brush strokes keep the legacy fixed-width polyline rendering because filled outlines cannot express dashes.
- Stamp (`S`) is a FigJam-style first-class reaction tool: an options bar next to the dock picks from eight stamps (+1, -1, heart, star, check, question, eyes, celebrate), the cursor becomes the picked stamp, and clicking places it on the board or on top of objects (stamps attach to the object underneath via `targetNodeId`). Press-and-hold grows the stamp through four wiggling stages before placing, matching FigJam. The tool stays active for continuous stamping; Esc or `V` exits. Stamps are `markup` nodes with `markupKind: 'stamp'` and `markupStampKind`, keep organization actions (move, duplicate, lock, hide, layer, delete) but hide text and stroke/fill styling, and the last-used stamp persists locally. FigJam's `E` shortcut is intentionally not mirrored because Mivo's `E` belongs to the brush eraser.
- Drawn shape markup can be placed directly over existing canvas objects, returns to Select after placement, and does not immediately show the purple edit frame. A second click selects it; arrow and line markup then expose endpoint handles for FigJam-style direction editing, while filled shapes use the normal bounding-box resize handles. Double-clicking any markup shape edits text inside that object instead of opening image details.
...
- Canvas node type knowledge now starts in `src/model/canvasNodeRegistry.ts`. Each node type declares its render kind, default size, import behavior, and capabilities there, while `src/model/nodeCapabilities.ts` owns the shared capability vocabulary.
...
- Useful references reviewed for this direction:
  - [FigJam quick bar study](figjam-quickbar-study.md), Mivo's public-behavior quick-bar specification and implementation checklist.
