...
for f in bench/baselines/0b-*.json; do
  [ -f "$f" ] || continue
  base=$(basename "$f")
  case "$base" in
    0b-invalid-*) ;;
    *) mv "$f" "bench/baselines/0b-invalid-$base" ;;
  esac
done
...
for(const f of fs.readdirSync("bench/baselines").filter(f=>f.startsWith("0b-")&&f.endsWith(".json"))){
  ...
  for(const c of r.configs) for(const d of c.dprResults){const m=d.median;
    rows.push({n:c.nodeCount,ren:r.protocol.renderer,cull:r.protocol.culling,dpr:d.dpr,
      ...
      leaferChildren:d.runs?.[0]?.renderState?.leaferChildren??"",
      leaferExpected:d.runs?.[0]?.renderState?.leaferExpectedChildren??"",
      leaferPixels:d.runs?.[0]?.renderState?.leaferPixelNonEmpty??""})}
}
rows.sort((a,b)=>a.n-b.n||a.ren.localeCompare(b.ren)||a.cull.localeCompare(b.cull)||a.dpr-b.dpr);
console.log("nodes|renderer|culling|dpr|loadFixture|renderSync|panP95|zoomP95|heapDelta|longTask|panLongTask|leaferChildren|leaferExpected|leaferPixelNonEmpty");
for(const r of rows)console.log(`${r.n}|${r.ren}|${r.cull}|${r.dpr}|${r.load}|${r.sync}|${r.panP95}|${r.zoomP95}|${r.heap}|${r.lt}|${r.panLt}|${r.leaferChildren}|${r.leaferExpected}|${r.leaferPixels}`);
'
