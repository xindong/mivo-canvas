...
describe('canvasActionModel — characterization · part 1 (① 结构 ② 矩阵 ③ contextMenu/section 分发)', () => {
  ...
  describe('contextMenu dispatch', () => {
    ...
    it('view-details · frame → setActiveTool(select) + onRenameNode(id)', () => {
      ...
      expect(vi.mocked(rt.onOpenDetails)).toBeUndefined()
    })
...
// BASELINE COUNT (part 1): part 1+2 合计 305 expect / 189 it（见 PR 描述）。迁移前后须一致；
