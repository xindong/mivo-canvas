...
// ── T2.2 Block 1:ai-slot 占位落 server + rollback 接 delete-node ──────────────────
// 验 chat slot(prepareChatSlot)+ mask-edit slot(prepareMaskEditPlaceholder)两 caller 经
// wrapMutationForScene → server 模式 create-node submitChange;失败/cancel rollback
// (removeMaskEditPlaceholder)→ delete-node;retry 复用 pendingSlotId 不重复建;local 模式 gate 不发。
// 注:非 mask 的 chat slot 失败时按设计保留(待 retry 复用,settleCanvasGenerationInState 只翻 status 不删
// node),故无"chat slot 失败删 slot"路径可接;唯一 slot-delete 路径是 mask-edit 的 removeMaskEditPlaceholder
// (chatMaskEditFlow 失败/取消两处均经它)。
describe('T2.2 Block 1 — ai-slot 占位落 server + rollback(delete-node)', () => {
    ...
    store.setState(
      {
        ...
        sceneId: 'c1',
        canvases: {
          c1: { title: 'Canvas', createdAt: '2026-07-15T00:00:00.000Z', updatedAt: '2026-07-15T00:00:00.000Z', nodes: [source], edges: [], tasks: [], selectedNodeId: 'src1', selectedNodeIds: ['src1'] },
        },
        nodes: [source],
        edges: [],
        tasks: [],
        selectedNodeId: 'src1',
        selectedNodeIds: ['src1'],
      } as never,
      true,
    )
  }
  ...
  it('server 模式:prepareMaskEditPlaceholder(mask slot)→ create-node;removeMaskEditPlaceholder(rollback)→ delete-node', async () => {
    const { __resetCanvasSyncRuntimeQueue, submitChange, useCanvasStore } = await loadRuntimeModule()
    ...
    seedCanvasWithSource(useCanvasStore, source)

    const { slotId, baselineSnapshot } = prepareMaskEditPlaceholder('c1', source, 'remove the cat')
    await drain()

    expect(submitChange).toHaveBeenCalledWith('c1', expect.objectContaining({ kind: 'create-node' }))
    const slotBefore = useCanvasStore.getState().canvases.c1.nodes.find((n) => n.id === slotId)
    expect(slotBefore?.type).toBe('ai-slot')
    ...
    const slotAfter = useCanvasStore.getState().canvases.c1.nodes.find((n) => n.id === slotId)
    expect(slotAfter).toBeUndefined()
    __resetCanvasSyncRuntimeQueue()
  })
  ...
  it('retry 复用 pendingSlotId → 不重复 create-node(同 slot id,server 已有)', async () => {
    const { __resetCanvasSyncRuntimeQueue, submitChange, useCanvasStore } = await loadRuntimeModule()
    const { generationFacade } = await import('../../store/generationFacade')
    seedEmptyCanvas(useCanvasStore)
