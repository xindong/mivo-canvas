...
// 提交装配:锚点方位编排 + 结构化提示词组装 + 红圈标注图构建。纯函数,
// 依赖通过参数进;recognitionsRef 以 ref 快照读取(与原 submit 闭包一致)。
import { boundsForRegions, type ImageMaskBounds, type ImageMaskRegion } from './imageMaskGeometry'
...
export async function buildMaskEditPromptBundle({
  body,
  regions,
  naturalSize,
  resolvedAssetUrl,
...
}): Promise<{ finalPrompt: string; markedImage: Awaited<ReturnType<typeof buildAnchorMarkedImage>> | null }> {
  ...
  const markedImage =
    markShapes.length && resolvedAssetUrl
      ...
      : null
  return { finalPrompt, markedImage }
}
