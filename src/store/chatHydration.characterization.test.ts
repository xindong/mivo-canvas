...
// 断言数 baseline: 106 expect() 调用点 / 29 it / 6 describe
...
describe('③ hydrate 后 settleExpiredChatMessages 对 in-flight 消息的确切回落', () => {
  it('enhancing 消息 → error / unknown / 过期文案 / chat retryDisabledReason=undefined', async () => {
    seedPersisted({
      ...
      paramOverrides: { imgRatio: 'auto', quality: 'auto' },
      messagesByScene: {
        'scene-e': [makeMessage({ id: 'e1', status: 'enhancing', text: '', origin: 'chat', generationContext: { model: 'gpt-image-2', requestedImgRatio: 'auto', requestedQuality: 'auto' } })],
      },
    })
    await useChatStore.persist.rehydrate()
