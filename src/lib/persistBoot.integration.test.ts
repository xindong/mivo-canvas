...
describe('Phase 1 项4 — 持久 tombstone 全生命周期(DELETE 离队后接力挡复活,CR-3/CR-4)', () => {
  ...
  it('P3:deleteProject 409 后权威确认 project/child 均删除 → 保持本地缺失、消费双 tombstone、无恢复 toast', async () => {
    localStorage.setItem('mivo:server-migration:anonymous', 'done')
    const serverProject = proj('pX', 'Server P')
    let childDeleteCalls = 0
    let scopedCanvasReads = 0
    const fetch = async (input: string, init?: RequestInit): Promise<Response> => {
      const url = new URL(input, 'http://stub')
      const method = (init?.method ?? 'GET').toUpperCase()
      const json = (body: unknown, status = 200) => new Response(JSON.stringify(body), {
      ...
      if (method === 'DELETE' && url.pathname === '/api/projects/pX') return json({ error: 'active-child', id: 'pX' }, 409)
      if (method === 'DELETE' && url.pathname === '/api/canvas/cX') {
        childDeleteCalls++
        return new Response(null, { status: 204 })
      }
      if (method === 'GET' && url.pathname === '/api/projects') return json({ projects: [] })
      if (method === 'GET' && url.pathname === '/api/canvas') {
        scopedCanvasReads++
        expect(url.searchParams.get('projectId')).toBe('pX')
    ...
    useCanvasStore.setState({
      canvases: {
        cX: ({ title: 'local', projectId: 'pX', status: 'active', createdAt: 't', updatedAt: 't', nodes: [], edges: [], tasks: [] }) as never,
        survivor: ({ title: 'survivor', createdAt: 't', updatedAt: 't', nodes: [], edges: [], tasks: [] }) as never,
      },
      sceneId: 'survivor',
    })
    ...
    await flush()
    await drainPersistQueue()
    expect(scopedCanvasReads).toBe(1)
    expect(childDeleteCalls).toBe(0)
  ...
  it('P1:active-child marker put 失败 + 即时 reconcile restored → 恢复 store、消费 tombstone、子画布 DELETE 零发出', async () => {
    localStorage.setItem('mivo:server-migration:anonymous', 'done')
    const serverProject = proj('pX', 'Server P')
    const serverCanvases: CanvasMeta[] = [
      { id: 'c1', projectId: 'pX', title: 'remote one', createdAt: 't', updatedAt: 't', metaRevision: 4, contentVersion: 0, status: 'active' },
      { id: 'c2', projectId: 'pX', title: 'remote two', createdAt: 't', updatedAt: 't', metaRevision: 5, contentVersion: 0, status: 'archived' },
    ]
    ...
    const fetch = async (input: string, init?: RequestInit): Promise<Response> => {
      ...
      if (method === 'DELETE' && url.pathname.startsWith('/api/canvas/')) {
        childDeleteCalls++
        return new Response(null, { status: 204 })
      }
      if (method === 'GET' && url.pathname === '/api/projects') {
        reconcileReads++
        return json({ projects: [serverProject] })
      }
      if (method === 'GET' && url.pathname === '/api/canvas') {
        reconcileReads++
        return json({ canvases: serverCanvases })
      }
      return json({})
    }
    const warn = vi.spyOn(toastFeedback, 'warn').mockImplementation(() => 'toast')
    ...
    const putSpy = vi.spyOn(FakeIDBObjectStore.prototype, 'put').mockImplementation(function (
      ...
      if (String(tombstone.key).endsWith(':project:pX') && tombstone.rollbackPending === true) {
        markerPutAttempts++
        operationOrder.push('marker-put-attempt')
        throw new Error('injected rollbackPending IDB put failure')
      }
      return key === undefined ? originalPut.call(this, value) : originalPut.call(this, value, key)
    })
    const getAllSpy = vi.spyOn(FakeIDBObjectStore.prototype, 'getAll').mockImplementation(function (
      this: InstanceType<typeof FakeIDBObjectStore>,
      ...args: Parameters<typeof originalGetAll>
    ) {
      // onOutcome 中 marker 尝试后的首个 writes 全量扫描就是 cancelDeleteCanvases。
      if (this.name === 'writes' && markerPutAttempts > 0 && !cancelScanObserved) {
        cancelScanObserved = true
        operationOrder.push('cancel-delete-canvases-scan')
      }
    ...
    try {
      ...
      expect(operationOrder).toEqual(['marker-put-attempt', 'cancel-delete-canvases-scan'])
      expect(reconcileReads).toBe(2)
      expect(childDeleteCalls).toBe(0)
      expect(await getPendingProjectDeletionRollbackIds()).not.toContain('pX')
      expect((await getDeletionTombstones('project')).has('pX')).toBe(false)
      expect(await getDeletionTombstones('canvas')).toEqual(new Set())
      expect(useCanvasStore.getState().projects.find((project) => project.id === 'pX')).toMatchObject({ name: 'Server P' })
      expect(useCanvasStore.getState().canvases.c1).toMatchObject({ title: 'remote one', status: 'active' })
      expect(useCanvasStore.getState().canvases.c2).toMatchObject({ title: 'remote two', status: 'archived' })
      expect(warn).toHaveBeenCalledWith('项目内还有活跃画布(可能来自其他设备),已恢复显示;请先归档或移动再彻底删除。')
    } finally {
      putSpy.mockRestore()
  ...
  it('P1:active-child marker put 失败 + 即时 reconcile 失败 → 保留 tombstone、ERROR 出声、子画布 DELETE 零发出', async () => {
    localStorage.setItem('mivo:server-migration:anonymous', 'done')
    const serverProject = proj('pX', 'Server P')
    let childDeleteCalls = 0
    let reconcileReads = 0
    const fetch = async (input: string, init?: RequestInit): Promise<Response> => {
      ...
      if (method === 'GET' && (url.pathname === '/api/projects' || url.pathname === '/api/canvas')) {
        reconcileReads++
    ...
    try {
      ...
      expect((await getDeletionTombstones('project')).has('pX')).toBe(true)
      expect(await getDeletionTombstones('canvas')).toEqual(new Set(['c1', 'c2']))
      expect(errorLog.mock.calls.some(([, message]) => String(message).includes('degraded dual failure'))).toBe(true)
      expect(warn).toHaveBeenCalledWith('项目删除被阻止,但重试状态保存和服务器恢复均失败;已拦截画布删除并保留删除标记,请刷新后重试。')
    } finally {
      putSpy.mockRestore()
  ...
  it('P2-3:active-child 回灌 GET 全失败 → 不假报恢复、保留 tombstone；后续成功 GET 收敛并清凭据', async () => {
    ...
    const serverProject = proj('pX', 'Server P')
    const serverCanvas: CanvasMeta = {
      id: 'cX', projectId: 'pX', title: 'remote', createdAt: 't', updatedAt: 't',
      metaRevision: 4, contentVersion: 0, status: 'active',
    }
    ...
    const fetch = async (input: string, init?: RequestInit): Promise<Response> => {
      ...
      if (method === 'DELETE' && url.pathname === '/api/projects/pX') return json({ error: 'active-child', id: 'pX' }, 409)
      if (method === 'DELETE' && url.pathname === '/api/canvas/cX') return new Response(null, { status: 204 })
      if (method === 'GET' && url.pathname === '/api/projects') {
        reconcileRound++
        return reconcileRound === 1 ? json({ error: 'unavailable' }, 503) : json({ projects: [serverProject] })
      }
      if (method === 'GET' && url.pathname === '/api/canvas') {
        return reconcileRound === 1 ? json({ error: 'unavailable' }, 503) : json({ canvases: [serverCanvas] })
      }
      return json({})
    }
    const warn = vi.spyOn(toastFeedback, 'warn').mockImplementation(() => 'toast')
    ...
    expect(warn).toHaveBeenCalledWith('项目删除被阻止,但服务器状态恢复失败;已保留重试状态,请稍后重试。')
    expect(warn).not.toHaveBeenCalledWith('项目内还有活跃画布(可能来自其他设备),已恢复显示;请先归档或移动再彻底删除。')

    await hydrateFromServer(adapter([serverProject], [serverCanvas]), hydrateOpts)
    expect(useCanvasStore.getState().projects.find((p) => p.id === 'pX')).toMatchObject({ name: 'Server P' })
    expect(useCanvasStore.getState().canvases.cX).toMatchObject({ projectId: 'pX', title: 'remote' })
    expect((await getDeletionTombstones('project')).has('pX')).toBe(false)
    expect((await getDeletionTombstones('canvas')).has('cX')).toBe(false)
    expect(await getPendingProjectDeletionRollbackIds()).not.toContain('pX')
    expect(warn).not.toHaveBeenCalledWith('项目内还有活跃画布(可能来自其他设备),已恢复显示;请先归档或移动再彻底删除。')
  ...
  it('P2-3:project GET 成功但 canvas GET 失败 → 零部分应用 + tombstone 保留 + 无假成功文案', async () => {
    localStorage.setItem('mivo:server-migration:anonymous', 'done')
    const serverProject = proj('pX', 'Server P')
    const fetch = async (input: string, init?: RequestInit): Promise<Response> => {
      const url = new URL(input, 'http://stub')
      ...
      if (method === 'DELETE' && url.pathname === '/api/canvas/cX') return new Response(null, { status: 204 })
      if (method === 'GET' && url.pathname === '/api/projects') return json({ projects: [serverProject] })
      if (method === 'GET' && url.pathname === '/api/canvas') return json({ error: 'canvas-unavailable' }, 503)
      return json({})
    }
    const warn = vi.spyOn(toastFeedback, 'warn').mockImplementation(() => 'toast')
    startPersistWriteQueue({ fetch, baseUrl: '', getAuthHeaders: () => authHeaders() })
  ...
  it('P2:rollbackPending + project/canvas 权威均空 → 消费 marker 与级联 tombstone；第二次 hydrate 不再专用 reconcile', async () => {
    ...
    let projectReads = 0
    let canvasReads = 0
    const log = vi.spyOn(debugLogger, 'log')
    const emptyAdapter = {
      listProjects: async () => {
        projectReads++
        return { projects: [] }
      },
      listCanvas: async (...args: Parameters<ServerPersistAdapter['listCanvas']>) => {
        canvasReads++
        if (args[0] !== undefined) expect(args).toEqual(['pX', { includeArchived: true }])
        return { canvases: [] }
      },
      listChatMessages: async () => ({ messages: [], orderRevision: 0 }),
    } as unknown as ServerPersistAdapter
    ...
    const childCommitLog = log.mock.calls.findIndex(([, message]) => String(message).includes('strictly revoked'))
    const projectCommitLog = log.mock.calls.findIndex(([, message]) => String(message).includes('tombstone cleared for project pX'))
    expect(childCommitLog).toBeGreaterThanOrEqual(0)
    expect(projectCommitLog).toBeGreaterThan(childCommitLog)
  ...
  it('P2:child tombstone IDB 中途删除失败 → marker 保留；下轮 hydrate 重试后收敛', async () => {
    localStorage.setItem('mivo:server-migration:anonymous', 'done')
    const previousIndexedDb = globalThis.indexedDB
    vi.stubGlobal('indexedDB', new FakeIDBFactory())
    await __resetDeletionTombstonesDb()
    await recordDeletionTombstone('project', 'pX')
    await recordDeletionTombstone('canvas', 'c1', { parentProjectId: 'pX' })
    await recordDeletionTombstone('canvas', 'c2', { parentProjectId: 'pX' })
    await markProjectDeletionRollbackPending('pX')
    resetStoreProjects([])
    useCanvasStore.setState({ canvases: {}, sceneId: '' as never })

    let specialCanvasReads = 0
    const emptyAdapter = {
      listProjects: async () => ({ projects: [] }),
      listCanvas: async (...args: Parameters<ServerPersistAdapter['listCanvas']>) => {
        if (args[0] !== undefined) {
          specialCanvasReads++
          expect(args).toEqual(['pX', { includeArchived: true }])
        }
        return { canvases: [] }
      },
      listChatMessages: async () => ({ messages: [], orderRevision: 0 }),
    ...
    let injected = false
    const deleteSpy = vi.spyOn(FakeIDBObjectStore.prototype, 'delete').mockImplementation(function (this: InstanceType<typeof FakeIDBObjectStore>, query) {
      if (!injected && String(query).endsWith(':canvas:c2')) {
        injected = true
        throw new Error('injected child tombstone IDB delete failure')
      }
    ...
    try {
      ...
      expect(await getPendingProjectDeletionRollbackIds()).toContain('pX')
      expect((await getDeletionTombstones('project')).has('pX')).toBe(true)
      expect((await getDeletionTombstones('canvas')).has('c1')).toBe(false)
      expect((await getDeletionTombstones('canvas')).has('c2')).toBe(true)

      deleteSpy.mockRestore()
      await hydrateFromServer(emptyAdapter, hydrateOpts)
      expect(specialCanvasReads).toBe(2)
      expect(await getPendingProjectDeletionRollbackIds()).not.toContain('pX')
      expect((await getDeletionTombstones('project')).has('pX')).toBe(false)
      expect((await getDeletionTombstones('canvas')).has('c2')).toBe(false)
    } finally {
      deleteSpy.mockRestore()
      await __resetDeletionTombstonesDb()
      vi.stubGlobal('indexedDB', previousIndexedDb)
  ...
  it('P3:rollbackPending project 缺失但 pX 仍有 archived child → includeArchived scoped 查询后不判权威删除', async () => {
    localStorage.setItem('mivo:server-migration:anonymous', 'done')
    await seedRollbackPendingDeletion()
    const archivedChild: CanvasMeta = {
      id: 'cX', projectId: 'pX', title: 'still live', createdAt: 't', updatedAt: 't',
      metaRevision: 1, contentVersion: 0, status: 'archived',
    }
    const inconsistentAdapter = {
      listProjects: async () => ({ projects: [] }),
      listCanvas: async (...args: Parameters<ServerPersistAdapter['listCanvas']>) => {
        if (args[0] !== undefined) expect(args).toEqual(['pX', { includeArchived: true }])
        return { canvases: [archivedChild] }
      },
      listChatMessages: async () => ({ messages: [], orderRevision: 0 }),
    } as unknown as ServerPersistAdapter
  ...
  it('P3:rollbackPending 仅收到其他 project child → 不影响 pX 权威删除判定', async () => {
    localStorage.setItem('mivo:server-migration:anonymous', 'done')
    await seedRollbackPendingDeletion()
    const otherProjectChild: CanvasMeta = {
      id: 'cY', projectId: 'pY', title: 'other', createdAt: 't', updatedAt: 't',
    ...
    const scopedAdapter = {
      ...
      listCanvas: async (...args: Parameters<ServerPersistAdapter['listCanvas']>) => {
        if (args[0] !== undefined) expect(args).toEqual(['pX', { includeArchived: true }])
        return { canvases: [otherProjectChild] }
      },
      listChatMessages: async () => ({ messages: [], orderRevision: 0 }),
    } as unknown as ServerPersistAdapter

    await hydrateFromServer(scopedAdapter, hydrateOpts)
    expect(await getPendingProjectDeletionRollbackIds()).not.toContain('pX')
    expect((await getDeletionTombstones('project')).has('pX')).toBe(false)
    expect((await getDeletionTombstones('canvas')).has('cX')).toBe(false)
  })
