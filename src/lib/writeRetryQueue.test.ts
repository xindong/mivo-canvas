...
describe('G1-a R5 F1 — 同毫秒 createCanvas/appendChatMessage 的 drain 依赖排序(三层 rank)', () => {
  ...
  it('手术刀守卫:chat 依赖的 canvas 已 preseed(无同毫秒 FK 竞争)→ 任一顺序双 success,三层 rank 不误伤', async () => {
    // 回归守卫:同毫秒 createProject + appendChatMessage,但 chat 依赖的 canvas c-pre 已在先前 drain 建好
    // (mock preseed)——chat 与同毫秒的 createProject 无 FK 竞争,任一 drain 顺序均双 success。
    // 此场景修复前(两层 rank,chat=0:chat 先发,canvas preseed → success)与修复后(三层 rank,chat=2:
    // project 先发,chat 后发)outcome 相同 → 守卫三层 rank 不误伤无 FK 依赖的 same-ms op(只在 tie 时重排)。
    ...
    const r = await q.drain()
    // 无 FK 竞争:canvas c-pre 已 preseed → chat 无论先/后发均 success,project 亦 success。
    // 手术刀守卫:三层 rank 只在 same-ms tie 时重排,不误伤无 FK 依赖的 same-ms op 的 outcome
    // (修复前 chat 先发亦 2 success / 修复后 project 先发亦 2 success → outcome 不变,只顺序变)。
    expect(calls).toHaveLength(2)
    expect(calls.map((c) => c.op.kind).sort()).toEqual(['appendChatMessage', 'createProject'])
    expect(r).toEqual({ processed: 2, successes: 2, failures: 0, terminals: 0, paused: false })
    expect(await q.pendingCount()).toBe(0)
  })
})
