...
describe('T1.3 ServerPersistAdapter ↔ server contract 类型共享互锁(返修版二)', () => {
  ...
  it('P1-2/F3 返修五:超长输入不卡死(无 % fixed-point 立即返回;多层编码累计输出长度阈值兜底,不无限循环)', () => {
    // 超长无 % 字符串:fixed-point 立即返回(单次循环,不卡死)
    expect(scanForSensitiveFields({ data: 'a'.repeat(200_000) })).toBeNull()
    // 多层编码超长:累计解码输出长度阈值兜底,在合理时间返回(不卡死;null 或 'data')
    const r = scanForSensitiveFields({ data: '%25'.repeat(50_000) + '61piKey' })
    expect(r === null || r === 'data').toBe(true)
  })
