...
//   时序确定性:onConflict 在 drain 行 1950 同步 fire rehydrate,rehydrate 首语句 `await import(canvasStore)`
//   产 microtask M1(置 global=true);drain 紧接 `await recordTerminal`(1956)+`await deleteWrite`(1957,
//   memStore async 各 yield microtask)→ M1 在 start 返回前落地 → 行 1170 读 global=true(修前 bug)。
//   2 条 stale update 提供额外 microtask gap(第二条的 fetch await)作安全余量。
...
const makeServer = (opts: { failListCanvasOnce?: boolean } = {}): {
  ...
  const fetch = async (input: string, init?: RequestInit): Promise<Response> => {
    ...
    if (method === 'PATCH' && path.startsWith('/api/projects/')) {
      // stale updateProject(baseRevision=0)→ 409 revision-conflict → onConflict fire rehydrate
...
describe('P1 r6 三轮终审 — collectionOk 快照时机(onConflict rehydrate 覆写 global 不致误种 marker)', () => {
  it('原 boot partial(step2 listCanvas 抛)+ start drain 撞 409 rehydrate 覆写 global=true → flush 用快照 false → marker=null(修前 done)', async () => {
    ...
    await flush(20) // 让 fire-and-forget rehydrate 完全落地(失败亦落地;marker 决策已在 bootPersistWiring 行 1170 完成)
    // 修前:flush 收到 global=true(rehydrate 覆写)→ allRecoverable && collectionOk → 种 marker='done'(bug)
    // 修后:flush 收到快照=false(boot partial 真值)→ 不种 marker → 下次 boot 重收集补 canvas
