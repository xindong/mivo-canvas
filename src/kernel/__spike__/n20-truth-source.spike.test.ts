...
class TextLwwWithOverwrite {
  ...
  /** 前写者 restore:发新 op value=historicalValue(恢复自己的值,新 seq,后写 wins)。 */
  restore(actor: string, notice: OverwrittenNotice): ApplyResult | { kind: 'forbidden' } {
    return this.server.applyOpAuthz(
...
describe('N2-0 返修 Gate1 文本判决(P1-4 二选一写死): B 方案 LWW 200 + overwritten + restore', () => {
  ...
  it('T1-2 restore 恢复前值:A 收 overwritten 后发 restore → title 回 A(新 seq,后写 wins)', () => {
    const s = new FieldLevelServer()
    s.seedNode(makeNode('n1', { title: 'orig' }))
    s.addMember('alice'); s.addMember('bob')
    const lww = new TextLwwWithOverwrite(s)
    ...
    // A 收 overwritten → restore(发 historicalValue=A,新 seq,后写 wins)
...
describe('N2-0 返修 §10 G1-b 衔接(P2-8): FieldPath 非空 + 数组中性 intent + create→edit 因果 + DELETE cursor', () => {
  it('S10-6 FieldPath 非空 tuple(G1B R2-P1-1 对齐):拒空路径 []', () => {
    ...
    const obj: Record<string, unknown> = { title: 'orig' }
    expect(() => setByPath(obj, [], 'x')).toThrow(/empty fieldPath/i)
  })

  it('S10-7 数组中性 intent by-stable-id(G1B R2-P1-1 对齐):insert/remove/splice 用 id 定位,非 index', () => {
    // G1B R2-P1-1:数组(fills/strokes/effects/markupPoints/resultNodeIds)无中性 intent;
    //   JS delete 留 sparse hole ≠ Y.Array.delete;index 在并发 insert 后漂移 → 删错元素。
    //   修法:数组 intent 用 stable-id 定位(insert afterId / remove id / splice byId),非 index。
    type ArrayIntent =
      | { kind: 'array'; recordId: string; fieldPath: (string | number)[]; intent: 'insert'; afterId: string | null; value: { id: string } }
      | { kind: 'array'; recordId: string; fieldPath: (string | number)[]; intent: 'remove'; removeId: string }
      | { kind: 'array'; recordId: string; fieldPath: (string | number)[]; intent: 'splice'; afterId: string; removeCount: number; values: { id: string }[] }
    const applyArrayIntent = (arr: { id: string }[], intent: ArrayIntent): { id: string }[] => {
      if (intent.intent === 'insert') {
        const idx = intent.afterId === null ? -1 : arr.findIndex((x) => x.id === intent.afterId)
        if (intent.afterId !== null && idx === -1) throw new Error(`afterId ${intent.afterId} not found`)
        const at = idx + 1
        return [...arr.slice(0, at), intent.value, ...arr.slice(at)]
      }
      if (intent.intent === 'remove') {
        return arr.filter((x) => x.id !== intent.removeId) // ★ by id,非 splice(index)(index 漂移风险)
      }
      const idx = arr.findIndex((x) => x.id === intent.afterId)
      if (idx === -1) throw new Error(`afterId ${intent.afterId} not found`)
      return [...arr.slice(0, idx + 1), ...intent.values, ...arr.slice(idx + 1 + intent.removeCount)]
    }
    const base = [{ id: 'f1' }, { id: 'f2' }]
    const aInsert = applyArrayIntent(base, { kind: 'array', recordId: 'n1', fieldPath: ['fills'], intent: 'insert', afterId: 'f1', value: { id: 'fA' } })
    expect(aInsert.map((x) => x.id)).toEqual(['f1', 'fA', 'f2'])
    // remove by id(非 index):即使并发 insert 改了 index,id 定位仍准
    const afterRemove = applyArrayIntent(aInsert, { kind: 'array', recordId: 'n1', fieldPath: ['fills'], intent: 'remove', removeId: 'fA' })
    expect(afterRemove.map((x) => x.id)).toEqual(['f1', 'f2'])
    // splice by afterId + removeCount(全 by-id 定位 afterId)
    const spliced = applyArrayIntent([{ id: 'f1' }, { id: 'f2' }, { id: 'f3' }], { kind: 'array', recordId: 'n1', fieldPath: ['fills'], intent: 'splice', afterId: 'f1', removeCount: 1, values: [{ id: 'fN' }] })
    expect(spliced.map((x) => x.id)).toEqual(['f1', 'fN', 'f3'])
    // ★ 对照:index-based remove 在并发 insert 后会删错;by-stable-id 不会(G1B R2-P1-1 要求)。
  })
