...
class FieldLevelServer {
  ...
  private idempotencyCache = new Map<string, ApplyResult>()
...
describe('N2-0 返修 §10: 三层信任边界 + typed op union + setByPath 防原型污染', () => {
  ...
  it('S10-2 三层信任边界(R2-3):trustify 注入 idempotency-key header 为 opId 单一权威载体;body 的 actor/recordId/base/opId 全弃', () => {
    // R2-3:body 零信任字段 — actor/recordId/baseRevision/opId 全部不可信;trusted 源:
    //   actor ← resolveActor(authz from gateway-secret+auth-user header);recordId ← URL path;baseRevision ← If-Match header;opId ← idempotency-key header
    // 客户端发的 op(body 可被客户端伪造,不可信)
    type ClientFieldOp = {
      opId: string; clientId: string
      actor: string          // ★ body 自带(不可信,弃)
      recordId: string       // ★ body 自带(不可信,弃)
      baseRevision: Revision // ★ body 自带(不可信,弃)
      fieldPath: (string | number)[]; value: unknown
    }
    // 服务端 trusted 形态:四 trusted 源覆盖,opId 单一权威载体(idempotency-key header)
    type TrustedFieldOp = {
      opId: string          // ← idempotency-key header(单一权威载体,弃 body.opId)
      clientId: string
      actor: string         // ← resolveActor(c.req)— trusted
      recordId: string      // ← URL path(:nodeId)— trusted
      baseRevision: Revision // ← If-Match header(parseIfMatch)— trusted
      fieldPath: (string | number)[]; value: unknown
    }
    // R2-3 修法:trustify 加 idempotencyKeyHeader 参数,opId 从 header 取(丢弃 body.opId)
    const trustify = (
      client: ClientFieldOp,
      ctx: { authzActor: string; pathRecordId: string; ifMatchBase: Revision; idempotencyKeyHeader: string },
    ): TrustedFieldOp => ({
      opId: ctx.idempotencyKeyHeader, // ★ header 单一权威载体(弃 body.opId)
      clientId: client.clientId,
      actor: ctx.authzActor,          // ★ 覆盖(不信 body.actor)
      recordId: ctx.pathRecordId,     // ★ 覆盖(不信 body.recordId)
      baseRevision: ctx.ifMatchBase,  // ★ 覆盖(不信 body.baseRevision)
      fieldPath: client.fieldPath, value: client.value,
    })
    // 客户端试图伪造 actor='admin' / recordId='other-node' / base=999 / opId='fake-body-opid'
    const malicious: ClientFieldOp = {
      opId: 'fake-body-opid', clientId: 'A', actor: 'admin', recordId: 'other-node', baseRevision: 999,
      fieldPath: ['title'], value: 'hacked',
    }
    const trusted = trustify(malicious, { authzActor: 'alice', pathRecordId: 'n1', ifMatchBase: 0, idempotencyKeyHeader: 'idem-key-abc' })
    ...
    expect(trusted.opId).toBe('idem-key-abc')       // ★ header 生效,body.opId='fake-body-opid' 被忽略(R2-3)
    expect(trusted.opId).not.toBe('fake-body-opid') // ★ 伪造 body opId 被丢弃
  })

  it('S10-3 typed domain op union + adapter 分层(R2-3/R2-4 对齐 G1B R2):DomainOp 中性 delta(无 recordId/actor/base/opId)→ adapter 注入 trusted → wire op', () => {
    // R2-3/R2-4:DomainOp 是中性 delta(transport-neutral),不带 recordId/actor/base/opId(这些全在 adapter 层注入);
    //   recordId 从 URL path(:nodeId),actor 从 resolveActor,base 从 If-Match,opId 从 idempotency-key header。
    //   对齐 G1B R2-P1-1:中性 intent 与 wire op 的 adapter 映射一对一写明。
    type DomainOp =
      | { kind: 'create'; recordId: string; type: 'node' | 'edge' | 'anchor'; payload: unknown } // create:recordId 是新 id(POST body 合理)
      | { kind: 'set'; fieldPath: (string | number)[]; value: unknown }      // ★ 无 recordId(adapter 从 path 注入)
      | { kind: 'unset'; fieldPath: (string | number)[] }
      | { kind: 'array'; fieldPath: (string | number)[]; class: 'by-id' | 'whole-lww' | 'primitive'; intent: 'insert' | 'remove' | 'splice' | 'replace'; afterId?: string | null; removeId?: string; value?: unknown } // 对齐 S10-7 三类
      | { kind: 'reorder'; orderedIds: string[] }                            // parentId 从 path
      | { kind: 'strict-tx'; ops: DomainOp[] } // 严格事务路径(跨 record 原子,§10.4)
    // adapter:中性 DomainOp + trusted 上下文 → wire op(recordId/actor/base/opId 全由 adapter 注入,R2-3)
    ...
    const set: DomainOp = { kind: 'set', fieldPath: ['title'], value: 'x' }      // ★ 中性(无 recordId)
    const unset: DomainOp = { kind: 'unset', fieldPath: ['tempKey'] }
    const arr: DomainOp = { kind: 'array', fieldPath: ['fills'], class: 'by-id', intent: 'insert', afterId: 'f1', value: { id: 'fA' } }
    ...
    // ★ R2-3 adapter 映射:中性 set → wire op(recordId/actor/base/opId 全由 trusted ctx 注入,不在 DomainOp)
    const wire = adaptToWire(set, { opId: 'idem-1', clientId: 'A', actor: 'alice', recordId: 'n1', baseRevision: 3 })
    expect(wire.recordId).toBe('n1')       // ★ path 注入(非 domain 自带)
    expect(wire.actor).toBe('alice')      // ★ authz 注入
    expect(wire.baseRevision).toBe(3)     // ★ If-Match 注入
    expect(wire.opId).toBe('idem-1')      // ★ idempotency-key 注入
  ...
  it('S10-4 per-field clock 持久形态(R2-3):PG field_clock schema + 客户端 base.clock 表达 + 重启可恢复', () => {
    // R2-3:per-field clock 不只是内存 Map — 须有 PG 持久 schema + 客户端 base 携带 clock 表达 + 重启可恢复
...
describe('N2-0 返修 §10 G1-b 衔接(P2-8): FieldPath 非空 + 数组中性 intent + create→edit 因果 + DELETE cursor', () => {
  ...
  it('S10-7 数组三类意图(对照 mivoCanvas.ts 真实类型,R2-4 对齐 G1B R2-P1-1):有 stable-id / 无 stable-id / primitive', () => {
    ...
    type ArrayIntent =
      | { kind: 'array'; recordId: string; fieldPath: (string | number)[]; class: 'by-id'; intent: 'insert'; afterId: string | null; value: { id: string } }
      | { kind: 'array'; recordId: string; fieldPath: (string | number)[]; class: 'by-id'; intent: 'remove'; removeId: string }
      | { kind: 'array'; recordId: string; fieldPath: (string | number)[]; class: 'by-id'; intent: 'splice'; afterId: string; removeCount: number; values: { id: string }[] }
      | { kind: 'array'; recordId: string; fieldPath: (string | number)[]; class: 'whole-lww'; intent: 'replace'; value: unknown[] } // ② 无 stable-id 整值 LWW
      | { kind: 'array'; recordId: string; fieldPath: (string | number)[]; class: 'primitive'; intent: 'insert'; value: string }      // ③ primitive insert by value
      | { kind: 'array'; recordId: string; fieldPath: (string | number)[]; class: 'primitive'; intent: 'remove'; value: string }      // ③ primitive remove by value
    ...
    const aInsert = applyArrayIntent(base, { kind: 'array', recordId: 'n1', fieldPath: ['fills'], class: 'by-id', intent: 'insert', afterId: 'f1', value: { id: 'fA' } })
    expect(aInsert.map((x) => (x as { id: string }).id)).toEqual(['f1', 'fA', 'f2'])
    const afterRemove = applyArrayIntent(aInsert, { kind: 'array', recordId: 'n1', fieldPath: ['fills'], class: 'by-id', intent: 'remove', removeId: 'fA' })
    expect(afterRemove.map((x) => (x as { id: string }).id)).toEqual(['f1', 'f2'])
    const spliced = applyArrayIntent([{ id: 'f1' }, { id: 'f2' }, { id: 'f3' }], { kind: 'array', recordId: 'n1', fieldPath: ['fills'], class: 'by-id', intent: 'splice', afterId: 'f1', removeCount: 1, values: [{ id: 'fN' }] })
    ...
    const ptsLww = applyArrayIntent(pts, { kind: 'array', recordId: 'n1', fieldPath: ['markupPoints'], class: 'whole-lww', intent: 'replace', value: [{ x: 3, y: 3 }] })
    ...
    const rids = ['n1', 'n2']
    const ridsIns = applyArrayIntent(rids, { kind: 'array', recordId: 'a1', fieldPath: ['resultNodeIds'], class: 'primitive', intent: 'insert', value: 'n3' })
    expect(ridsIns).toEqual(['n1', 'n2', 'n3'])
    const ridsRm = applyArrayIntent(ridsIns, { kind: 'array', recordId: 'a1', fieldPath: ['resultNodeIds'], class: 'primitive', intent: 'remove', value: 'n2' })
  ...
  it('S10-11 idempotent replay(R2-3):同 opId(idempotency-key)replay 不二次 bump revision/seq、不二次发事件', () => {
    // R2-3:idempotent replay 缺失 → 返修:同 opId 重放 revision/seq 不变、不二次发事件
    const s = new FieldLevelServer()
    s.seedNode(makeNode('n1'))
    s.addMember('alice')
    const rev0 = s.revision('n1')
