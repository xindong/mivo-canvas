...
// ════════════════════════════════════════════════════════════════════════════
// N2-0 v5 唯一契约权威类型(模块级,供跨 describe 共享;对齐 G1-b R4 + sol 第四轮 4 阻断)
// ════════════════════════════════════════════════════════════════════════════
// v5 教训:测试绿但没证明文字声称的语义(type-cast 冒充协议行为)。本轮每条修复让测试真跑该语义。
// v5 决议收口(4 阻断):Blocker 1 base.clock 完整 wire 生命周期(真 codec+签名+签发/更新);Blocker 2 文档全量一致化 + container 白名单取消(leaf-level set);Blocker 3 FX-5 迁移走 legacy 全量 record 兼容通道(旧 upsert=replace,backend.ts:1086-1088);Blocker 4 Gate5 finite short-poll 真模式。
type FieldPath = readonly [string | number, ...(string | number)[]]  // 非空 tuple(S10-6 运行时拒空)

// ── Blocker 1:base.clock 单一 wire = opaque BaseCursor(真 string codec + HMAC 签名,非 type-cast)──
//   生命周期:token 从 accepted {id,revision,seq,base} 响应签发 + hydrate snapshot 签发;client 持 opaque string 回传 If-Match;
//   server decodeBase 验签 → {rev};malformed/unsigned → 400;conflict 响应返 current base 供 re-fetch。
//   业务层只持 opaque string,codec 只在 adapter(SnapshotCursor opaque 规则同)。
const BASE_SECRET = 'test-base-secret' // 测试 fixture;真实 adapter 用 server secret + HMAC(未入 bundle)
/** 纯 JS keyed hash(测试用,非密码学安全;真实 adapter 用 node:crypto HMAC——src tsconfig 无 node types 故用 FNV-1a keyed hash 代)。 */
...
/** BaseCursor = opaque string token(branded;client 不可构造/伪造,server 签发)。 */
type BaseCursor = string & { readonly __brand: 'BaseCursor' }
/** adapter codec:encode rev → signed opaque token(签发 from accepted/snapshot)。 */
const encodeBase = (rev: number, cv?: number): BaseCursor => {
  const payload = cv !== undefined ? `r=${rev};c=${cv}` : `r=${rev}`
  return `base:${payload}.${baseSig(payload)}` as BaseCursor
}
/** adapter codec:decode + 验签 → {rev, cv?} | null(malformed/unsigned/tampered → null → 400)。 */
const decodeBase = (token: BaseCursor | string | undefined): { rev: number; cv?: number } | null => {
  ...
  if (sig !== baseSig(payload)) return null  // 签名错/篡改 → null(防篡改)
  const m = /^r=(\d+)(?:;c=(\d+))?$/.exec(payload)
  if (!m) return null
  return { rev: Number(m[1]), cv: m[2] !== undefined ? Number(m[2]) : undefined }
}
...
describe('N2-0 返修 §10: 三层信任边界 + typed op union + setByPath 防原型污染', () => {
  ...
  it('S10-2 三层信任边界(v5:PATCH body 零 privileged;base=opaque BaseCursor string codec;create client-id;by-id deferred;container 白名单取消)', () => {
    ...
    // ★ Blocker 1:真 string codec round-trip(encode → token → decode 验签 → {rev};非 type-cast)
    const base0 = encodeBase(0)
    expect(typeof base0).toBe('string')              // ★ BaseCursor 是 string(opaque token,client 持 opaque string)
    expect(decodeBase(base0)).toEqual({ rev: 0 })     // ★ decode 验签成功 → {rev:0}(真 round-trip)
    expect(decodeBase('base:r=0.deadbeefdeadbeef')).toBeNull()  // ★ 签名错 → null(防篡改)
    expect(decodeBase('not-a-base-token')).toBeNull()            // ★ malformed → null(400)
    expect(decodeBase(undefined)).toBeNull()                    // missing → null(428)
  ...
  it('S10-3 typed domain op union + adapter 分层(v4:create client-id 独立 endpoint;strict-tx 剔出 DomainOp 改 server-named invariant;三类 array 同一 adapter 映射)', () => {
    ...
    const createBody: CreateBody = { clientId: 'A', type: 'node', payload: { title: 'new' } }
    const createWire = trustifyCreate(createBody, { opId: 'idem-create', clientId: 'A', actor: 'alice', recordId: 'n-client-2', base: encodeBase(0) })
    ...
    const base3 = encodeBase(3)
    expect(decodeBase(base3)).toEqual({ rev: 3 })  // ★ Blocker 1:真 codec round-trip
...
describe('N2-0 返修 §10 G1-b 衔接(P2-8): FieldPath 非空 + 数组中性 intent + create→edit 因果 + DELETE cursor', () => {
  ...
  // ════════════════════════════════════════════════════════════════════════════
  it('S10-12 base.clock 完整 wire 生命周期 + base-driven 冻结矩阵(v5 Blocker 1:真 codec round-trip;delete/reorder 新鲜 base→200,并发 race→409;edit stale→200+overwritten;create dup→409;malformed→400)', () => {
    // v5 教训:上轮 MatrixHarness "record 存在就 409 delete""顺序不同就 409 reorder" 把正常操作判成 race → 推倒重写为 base-DRIVEN。
    //   base = opaque BaseCursor string(真 codec+签名,非 type-cast);decodeBase 验签 → {rev}/{cv}。
    //   生命周期:accepted/snapshot 签发 base;client 回传 If-Match;server decode;malformed/unsigned→400;conflict 返 current base 供 re-fetch。
    //   冻结矩阵:edit stale→200+overwritten(永非 409,G4-4);delete/reorder 新鲜 base→200,并发 race(stale base/dup id/orderedIds≠live)→409;create dup→409;legacy flag-off→400(见 C-1)。
    type Outcome = { status: number; outcome: 'accepted' | 'conflict' | 'rejected'; base?: BaseCursor; overwritten?: boolean }
    class BaseDrivenHarness {
      private recs = new Map<string, { rev: number; present: boolean; lastWriter: string }>()
      private cv = 0
      private children: string[] = []
      private outbox: { to: string; fp: FieldPath; by: string; curRev: number }[] = []
      /** hydrate snapshot 签发 record base(rev)。 */
      snapshot(nodeId: string): BaseCursor | null {
        const r = this.recs.get(nodeId); return r && r.present ? encodeBase(r.rev) : null
      }
      /** reorder base 签发(cv,canvas contentVersion)。 */
      snapshotOrder(): BaseCursor { return encodeBase(0, this.cv) }
      seedChildren(ids: string[]) { this.children = [...ids]; this.cv = 0 }
      /** edit:malformed→400;always 200 + overwritten(if stale);never 409(G4-4)。 */
      edit(nodeId: string, op: { fieldPath: FieldPath; value: unknown }, base: BaseCursor, actor: string): Outcome {
        const d = decodeBase(base)
        if (!d) return { status: 400, outcome: 'rejected' }  // malformed/unsigned → 400
        const r = this.recs.get(nodeId)
        if (!r || !r.present) return { status: 404, outcome: 'rejected' }
        const stale = d.rev < r.rev
        if (stale && r.lastWriter) this.outbox.push({ to: r.lastWriter, fp: op.fieldPath, by: actor, curRev: r.rev + 1 })
        r.rev += 1; r.lastWriter = actor
        return { status: 200, outcome: 'accepted', base: encodeBase(r.rev), overwritten: stale }  // ★ stale→200+overwritten(非 409)
      }
      /** delete:malformed→400;fresh base(rev===current)→200(removed);stale base(rev<current)→409 race。 */
      delete(nodeId: string, base: BaseCursor): Outcome {
        const d = decodeBase(base)
        if (!d) return { status: 400, outcome: 'rejected' }
        const r = this.recs.get(nodeId)
        if (!r || !r.present) return { status: 404, outcome: 'rejected' }
        if (d.rev === r.rev) { r.present = false; return { status: 200, outcome: 'accepted' } }  // ★ fresh base→200(removed,非 409)
        return { status: 409, outcome: 'conflict', base: encodeBase(r.rev) }  // ★ stale base→409(race;返 current base)
      }
      /** reorder:malformed→400;fresh cv + orderedIds=live 置换→200(即使顺序变);stale cv / orderedIds≠live→409。 */
      reorder(orderedIds: string[], base: BaseCursor): Outcome {
        const d = decodeBase(base)
        if (!d || d.cv === undefined) return { status: 400, outcome: 'rejected' }
        if (d.cv !== this.cv) return { status: 409, outcome: 'conflict', base: this.snapshotOrder() }  // ★ stale cv→409
        ...
        if (!validPerm) return { status: 409, outcome: 'conflict', base: this.snapshotOrder() }  // orderedIds≠live→409
        this.children = [...orderedIds]; this.cv += 1  // ★ fresh base + valid → 200(顺序变也成功)
        return { status: 200, outcome: 'accepted', base: this.snapshotOrder() }
      }
      /** create:dup id→409;new→201 + issue base。 */
      create(nodeId: string): Outcome {
        const r = this.recs.get(nodeId)
        if (r && r.present) return { status: 409, outcome: 'conflict', base: encodeBase(r.rev) }  // ★ dup id→409
        this.recs.set(nodeId, { rev: 1, present: true, lastWriter: '' })
        return { status: 201, outcome: 'accepted', base: encodeBase(1) }
      }
      drainOverwritten(to: string) { const e = this.outbox.filter((o) => o.to === to); this.outbox = this.outbox.filter((o) => o.to !== to); return e }
    }
    const h = new BaseDrivenHarness()
    // ── 新鲜 base:正常操作必须成功(非 race)──
    expect(h.create('n1').status).toBe(201)  // create n1 → rev 1
    const snap1 = h.snapshot('n1')!  // hydrate 签发 base(rev 1)
    expect(decodeBase(snap1)).toEqual({ rev: 1 })  // ★ 真 codec round-trip
    // edit fresh base → 200 + new base(rev 2)
    const e1 = h.edit('n1', { fieldPath: ['title'], value: 'A' }, snap1, 'alice')
    expect(e1.status).toBe(200); expect(e1.outcome).toBe('accepted'); expect(decodeBase(e1.base!)).toEqual({ rev: 2 })
    expect(e1.overwritten).toBe(false)
    // ★ edit stale base(snap1=rev1, current=rev2)→ 200 + overwritten(非 409,G4-4)
    const e2 = h.edit('n1', { fieldPath: ['title'], value: 'B' }, snap1, 'bob')
    expect(e2.status).toBe(200); expect(e2.outcome).toBe('accepted')  // ★ stale→200 非 409
    expect(e2.overwritten).toBe(true)
    expect(h.drainOverwritten('alice').length).toBe(1)  // alice 收 overwritten(败方知情)
    // ★ delete fresh base → 200(removed,非 409 即使 record 存在)— 上轮错判为 race,推倒重写
    const snap3 = h.snapshot('n1')!  // rev 3
    const d1 = h.delete('n1', snap3)
    expect(d1.status).toBe(200); expect(d1.outcome).toBe('accepted')  // ★ fresh base delete→200
    // ★ delete stale base → 409(race:record changed since base)
    h.create('n2')
    const snapN2a = h.snapshot('n2')!  // rev 1
    h.edit('n2', { fieldPath: ['title'], value: 'X' }, snapN2a, 'alice')  // bump rev 1→2
    const d2 = h.delete('n2', snapN2a)  // stale base(rev1 < current rev2)
    expect(d2.status).toBe(409); expect(d2.outcome).toBe('conflict')  // ★ stale delete→409(race)
    expect(decodeBase(d2.base!)).toEqual({ rev: 2 })  // conflict 返 current base 供 re-fetch
    // malformed/unsigned base → 400
    expect(h.delete('n2', 'not-a-base' as BaseCursor).status).toBe(400)
    expect(h.delete('n2', 'base:r=1.deadbeef' as BaseCursor).status).toBe(400)  // 签名错→400
    // create dup → 409
    expect(h.create('n2').status).toBe(409)
    // ★ reorder fresh base + 顺序变 → 200(非 409 即使顺序不同)— 上轮错判为 race,推倒重写
    h.seedChildren(['a', 'b', 'c'])
    const orderBase = h.snapshotOrder()  // cv 0
    expect(decodeBase(orderBase)).toEqual({ rev: 0, cv: 0 })
    const r1 = h.reorder(['c', 'b', 'a'], orderBase)  // 顺序变,但 fresh base → 200
    expect(r1.status).toBe(200); expect(r1.outcome).toBe('accepted')  // ★ fresh reorder→200(顺序变也成功)
    expect(decodeBase(r1.base!)).toEqual({ rev: 0, cv: 1 })  // bump cv, issue new base
    ...
    const orderBase2 = h.snapshotOrder()  // cv 1
    const r3 = h.reorder(['a', 'b', 'd'], orderBase2)  // 'd' 不在 live set
    expect(r3.status).toBe(409); expect(r3.outcome).toBe('conflict')  // orderedIds≠live→409
    // 冻结矩阵(决策 §14.1 引用):edit stale 200(非 409);delete/reorder fresh→200, stale→409;create dup→409;malformed→400
    type OpClass = 'edit-stale' | 'edit-fresh' | 'delete-fresh' | 'delete-stale' | 'reorder-fresh' | 'reorder-stale' | 'create-dup' | 'malformed'
    const MATRIX: Record<OpClass, { status: number; outcome: 'accepted' | 'conflict' | 'rejected' }> = {
      'edit-stale': { status: 200, outcome: 'accepted' },      // ★ G4-4:edit stale 永非 409
      'edit-fresh': { status: 200, outcome: 'accepted' },
      'delete-fresh': { status: 200, outcome: 'accepted' },    // ★ fresh delete→200(非 race)
      'delete-stale': { status: 409, outcome: 'conflict' },    // ★ stale delete→409(race)
      'reorder-fresh': { status: 200, outcome: 'accepted' },   // ★ fresh reorder→200(顺序变也成功)
      'reorder-stale': { status: 409, outcome: 'conflict' },   // ★ stale cv / orderedIds≠live→409
      'create-dup': { status: 409, outcome: 'conflict' },      // ★ dup id→409
      'malformed': { status: 400, outcome: 'rejected' },       // ★ malformed/unsigned base→400(非 409)
    }
    expect(Object.keys(MATRIX).sort()).toEqual(['create-dup', 'delete-fresh', 'delete-stale', 'edit-fresh', 'edit-stale', 'malformed', 'reorder-fresh', 'reorder-stale'])
    // ★ Blocker 1 验收:base 真 codec round-trip + 生命周期(accepted/snapshot 签发,conflict 返 current)+ base-driven 矩阵(正常 op 新鲜 base→200,race 才 409;edit stale→200+overwritten 非 409;malformed→400)。
  })
...
describe('N2-0 v5 cutover contract harness(Blocker 3:真实 WriteOp + NodePayload;upsert→legacy 全量 record 兼容通道 非域 DomainOps;flag decoder + rollback snapshot)', () => {
  // v5 教训:payloadToDomainOps 数组一刀切 whole-lww + container 白名单绕 by-id defer → 推倒重写。
  //   仓库事实:旧 upsert = 整 record REPLACE(server/persist/backend.ts:1086-1088 `payload: clone(payload)`,非 merge)。
  //   立场:旧 queued upsertNode 不可翻译为 DomainOps(replace ≠ field-level;delta-inversion 无算法);
  //     migration 走 legacy 全量 record 兼容通道(drain-only,whole-record replace 同 backend.ts);
  //     by-id 数组(fills/strokes/effects/experimentalAnchors)A2 deferred,migration 不绕过(不发明 by-id DomainOp,不降级整数组 LWW)。
  //   缺失 optional 字段:旧 upsert=replace → 缺失字段 = 移除(replace 覆盖,非 unset 生成,非 merge 保留)。
  ...
  const _noIdInPayload: HasId = false  // ★ id 不在 payload(若 id 回塞 → HasId=true → true=false build fail)

  // migration-on-read:旧 queued WriteOp → 新 wire(legacy-replace / delete / reorder 三类)
  type MigratedOp =
    | { kind: 'legacy-replace'; nodeId: string; payload: NodePayload }   // upsertNode → legacy 全量 record 兼容通道(replace,非 DomainOps)
  ...
  const migrateWriteOp = (op: WriteOp): MigratedOp => {
    switch (op.kind) {
      case 'upsertNode':
        // ★ 旧 upsert=整 record replace(backend.ts:1086-1088)→ 不可翻译为 DomainOps(replace≠field-level);
        //   migration 走 legacy 全量 record 兼容通道(whole-record replace);by-id deferred 不绕过(不发明 by-id/whole-lww DomainOp)。
        return { kind: 'legacy-replace', nodeId: op.nodeId, payload: op.payload }
      case 'deleteNode':
      ...
      default:
        throw new Error(`migrateWriteOp: kind ${op.kind} not in 3 classes (upsert→legacy-replace / delete / reorder)`)
    }
  }
  ...
  class CutoverHarness {
    ...
    /** PATCH decoder:flag-on DomainOp(leaf-level set);flag-off NodePayload(legacy upsert replace);反之 400 payload-rejected。 */
    patch(nodeId: string, body: LegacyUpsertBody | NewOp): { status: number; body: { error?: string; id?: string; revision?: number; seq?: number } } {
      ...
      if (this.flag) {
        ...
        if (op.kind === 'set') setByPath(r as Record<string, unknown>, [...op.fieldPath] as (string | number)[], op.value, { allowContainerClobber: false })  // leaf-level(container set 拒,白名单取消)
        else if (op.kind === 'array' && op.class === 'whole-lww') setByPath(r as Record<string, unknown>, [...op.fieldPath] as (string | number)[], op.value, { allowContainerClobber: true })  // 整数组 replace(whole-lww)
        r.revision += 1
        this.recs.set(nodeId, structuredClone(r))
        return { status: 200, body: { id: nodeId, revision: r.revision, seq: r.revision } }
      }
      // flag off:NodePayload 接受(legacy upsert replace);DomainOp → 400
      if (isNew) return { status: 400, body: { error: 'payload-rejected' } }
      this.recs.set(nodeId, structuredClone({ ...(body as LegacyUpsertBody), id: nodeId, revision: 1 }) as NodeRecord)
      return { status: 200, body: { id: nodeId, revision: 1, seq: 1 } }
    }
    /** ★ FX-5 drain 兼容通道:whole-record replace(同 backend.ts:1086-1088 `payload: clone(payload)`,非 merge)。 */
    drainLegacyUpsert(nodeId: string, payload: NodePayload) {
      const existing = this.recs.get(nodeId)
      const rev = existing ? existing.revision + 1 : 1
      // ★ whole-record replace:payload 覆盖全字段(缺失字段 = 移除,非 merge 保留);id+revision 来自 path/server(非 payload)
      this.recs.set(nodeId, structuredClone({ ...payload, id: nodeId, revision: rev }) as NodeRecord)
    }
    applyDelete(cmd: ServerInvariantCommand) {
      if (cmd.kind === 'node-delete-cascade') this.recs.delete(cmd.nodeId)  // server-named cascade(实证 PG-T1~T3/T7)
    }
    snapshot(): NodeRecord[] { return [...this.recs.values()].map((r) => structuredClone(r)) }
    /** rollback materialize:从 authoritative 全 record 重建 NodePayload(剥 id+revision;非 delta 反演)。 */
    materializeLegacyBody(nodeId: string): NodePayload | null {
      const r = this.recs.get(nodeId)
      if (!r) return null
      const { id: _id, revision: _rev, ...rest } = r
      void _id; void _rev
  ...
  it('C-2 FX-5 migration-on-read(v5:upsert→legacy 全量 record 兼容通道 非域 DomainOps;whole-record replace deep-equal;by-id deferred 不绕过)', () => {
    const h = new CutoverHarness()
    h.setFlag(true)
    expect(_noIdInPayload).toBe(false)  // ★ id 不在 NodePayload
    // ★ ① upsertNode → migrate → {kind:'legacy-replace'}(非 DomainOps;旧 upsert=replace,backend.ts:1086-1088)
    ...
    expect(m.kind).toBe('legacy-replace')  // ★ upsert → legacy-replace(非 edit/set DomainOps;by-id deferred 不绕过)
    if (m.kind === 'legacy-replace') {
      // ★ drain 兼容通道:whole-record replace;materialized record deep-equal payload(漏任一字段必红)
      h.drainLegacyUpsert(m.nodeId, m.payload)
      const got = h.get('n-up')!
      expect(got.id).toBe('n-up')           // ★ id 来自 WriteOp.nodeId(非 payload)
      expect(got.title).toBe('drained')     // ★ 漏 title → 红
      ...
      expect(got.fills).toEqual([])       // ★ 漏 fills → 红
      // ★ whole-record replace(非 merge):materialized payload deep-equal 原 payload(无 seed 残留字段)
      expect(h.materializeLegacyBody('n-up')).toEqual(payload)
    }
    // ★ ② replace 覆盖(非 merge):seed record 有 text='hello';drain payload 缺 text → materialized text=undefined(移除,非 merge 保留)
    h.seedRecord(makeNode('n-replace', { text: 'hello', title: 'seed' }))
    const payloadNoText = nodePayload({ title: 'no-text' })
    delete (payloadNoText as { text?: string }).text  // ★ 显式移除 text(makeNode 默认 text='hello';nodePayload spread 不删字段,须 delete)
    expect((payloadNoText as { text?: string }).text).toBeUndefined()  // payload 缺 text
    const m2 = migrateWriteOp({ kind: 'upsertNode', canvasId: 'c1', nodeId: 'n-replace', payload: payloadNoText, baseRevision: 0 })
    expect(m2.kind).toBe('legacy-replace')
    if (m2.kind === 'legacy-replace') {
      h.drainLegacyUpsert(m2.nodeId, m2.payload)
      const got = h.get('n-replace')!
      expect(got.title).toBe('no-text')
      expect((got as { text?: string }).text).toBeUndefined()  // ★ replace 移除 text(非 merge 保留 'hello')— 证 replace 语义
      expect(h.materializeLegacyBody('n-replace')).toEqual(payloadNoText)  // deep-equal(无残留)
    }
    // ★ ③ deleteNode → migrate → {kind:'delete',cmd:node-delete-cascade};applyDelete 后移除
    ...
    if (mDel.kind === 'delete') {
      expect(mDel.cmd.kind).toBe('node-delete-cascade')
      expect(h.recordExists('n-del')).toBe(true)
      h.applyDelete(mDel.cmd)
      expect(h.recordExists('n-del')).toBe(false)
    }
    // ★ ④ reorderChildren → migrate → {kind:'reorder',op:DomainOp reorder}
    ...
    if (mRe.kind === 'reorder') {
      expect(mRe.op.kind).toBe('reorder')
      expect(mRe.op.kind === 'reorder' && mRe.op.orderedIds).toEqual(['n3', 'n1', 'n2'])
    }
    // ★ Blocker 3 验收:旧 upsert=replace(backend.ts:1086-1088)→ migration 走 legacy 全量 record 兼容通道(非 DomainOps);by-id deferred 不绕过;whole-record replace deep-equal;缺失字段=移除(非 unset/merge);delete→cascade;reorder→DomainOp。
  })
