...
- Brush follows the FigJam/Excalidraw drawing model: the tool stays active after each stroke for continuous drawing (Esc or `V` exits), and an options bar next to the dock picks marker vs highlighter, three stroke widths (2/4/8), and stroke color from the shared markup palette. The last-used brush style is remembered in local persistence. Strokes render as pressure-aware variable-width filled paths through the MIT-licensed `perfect-freehand` library with Excalidraw's tuning; real stylus pressure is recorded per point for pen input while mouse strokes use velocity-simulated pressure. Highlighter strokes are a wider, uniform-width, semi-transparent variant stored as `markupBrushKind`. Dashed brush strokes keep the legacy fixed-width polyline rendering because filled outlines cannot express dashes.
