...
- New user-facing features must emit Debug Log entries through `debugLogger` and follow [Development Logging Rule](docs/development-logging.md): successful actions use `log`, unavailable/skipped/recoverable paths use `warning`, and failures use `error`.
